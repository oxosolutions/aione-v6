...
class Aione_GoogleMap {
	...
	public function __construct() {
		...
		add_filter( 'oxo_attr_aione-google-map', array( $this, 'attr' ) );
	}
	...
	function render_map( $args, $content = '' ) {
		...
		if ( $address ) {
			...
			if ( $map_style == 'theme' ) {
				...
				$animation                = 'yes';
				$infobox                  = 'custom';
				$infobox_background_color = oxo_hex2rgb( Aione()->theme_options['primary_color'] );
				$infobox_background_color = 'rgba(' . $infobox_background_color[0] . ', ' . $infobox_background_color[1] . ', ' . $infobox_background_color[2] . ', 0.8)';
				$overlay_color            = Aione()->theme_options['primary_color'];
				$brightness_level         = $this->calc_color_brightness( Aione()->theme_options['primary_color'] );
				$infobox_text_color       = ( $brightness_level > 140 ) ? '#fff' : '#747474';
			}

			if ( 'theme' == $icon && 'custom' == $map_style ) {
				for ( $i = 0; $i < $num_of_addresses; $i++ ) {
					$icon_array[$i] = get_template_directory_uri() . '/assets/images/aione_map_marker.png';
				}
			}
			...
			<script type="text/javascript">
				...
				function oxo_run_map_<?php echo $map_id ; ?>() {
					jQuery('#<?php echo $map_id ; ?>').oxo_maps({
						...
						overlay_color_hsl: <?php echo json_encode( oxo_rgb2hsl( $overlay_color ) ); ?>,
			...
			<?php
			if ( $defaults['id'] ) {
				$html = ob_get_clean() . sprintf( '<div id="%s"><div %s></div></div>', $defaults['id'], $this->attributes( 'aione-google-map' ) );
			} else {
				$html = ob_get_clean() . sprintf( '<div %s></div>', $this->attributes( 'aione-google-map' ) );
			}
	...
	function attr() {
		...
		$attr['class'] = 'shortcode-map oxo-google-map aione-google-map';
