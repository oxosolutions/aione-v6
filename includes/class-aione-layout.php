...
class Aione_Layout {
	...
	public function add_sidebar() {
		...
		if ( Aione()->template->has_sidebar() && ! Aione()->template->double_sidebars() ) {
			add_action( 'oxo_after_content', array( $this, 'append_sidebar_single' ) );
		} elseif ( Aione()->template->double_sidebars() ) {
			add_action( 'oxo_after_content', array( $this, 'append_sidebar_double' ) );
		} elseif ( ! Aione()->template->has_sidebar() && ( is_page_template( 'side-navigation.php') || is_singular( 'tribe_events' ) ) ) {
			add_action( 'oxo_after_content', array( $this, 'append_sidebar_single' ) );
		}
	...
	public function sidebar_options() {
		if ( is_home() ) {
			$sidebars = array(
				...
				'position'  => Aione()->theme_options[ 'blog_sidebar_position' ],
			);
		} elseif ( function_exists( 'is_bbpress' ) && is_bbpress() ) {
			$sidebars = array(
				'global'    => Aione()->settings->get( 'bbpress_global_sidebar' ),
				'sidebar_1' => Aione()->settings->get( 'ppbress_sidebar' ),
				'sidebar_2' => Aione()->settings->get( 'ppbress_sidebar_2' ),
				'position'  => Aione()->settings->get( 'bbpress_sidebar_position' ),
			);
			...
			$sidebars = array(
				'global'    => Aione()->theme_options[ 'woo_global_sidebar' ],
				'sidebar_1' => Aione()->theme_options[ 'woo_sidebar' ],
				'sidebar_2' => Aione()->theme_options[ 'woo_sidebar_2' ],
				'position'  => Aione()->theme_options[ 'woo_sidebar_position' ],
			);
			...
			$sidebars = array(
				'global'    => Aione()->theme_options[ 'pages_global_sidebar' ],
				'sidebar_1' => Aione()->theme_options[ 'pages_sidebar' ],
				'sidebar_2' => Aione()->theme_options[ 'pages_sidebar_2' ],
				'position'  => Aione()->theme_options[ 'default_sidebar_pos' ],
			);
		} elseif ( is_single() ) {
			$sidebars = array(
				'global'    => Aione()->theme_options[ 'posts_global_sidebar' ],
				'sidebar_1' => Aione()->theme_options[ 'posts_sidebar' ],
				'sidebar_2' => Aione()->theme_options[ 'posts_sidebar_2' ],
				'position'  => Aione()->theme_options[ 'blog_sidebar_position' ],
			);

			if ( is_singular( 'aione_portfolio' ) ) {
				$sidebars = array(
					'global'    => Aione()->theme_options[ 'portfolio_global_sidebar' ],
					'sidebar_1' => Aione()->theme_options[ 'portfolio_sidebar' ],
					'sidebar_2' => Aione()->theme_options[ 'portfolio_sidebar_2' ],
					'position'  => Aione()->theme_options[ 'portfolio_sidebar_position' ],
				);
			} else if ( is_singular( 'tribe_events' ) || is_singular( 'tribe_organizer' ) || is_singular( 'tribe_venue' ) ) {
				$sidebars = array(
					'global'    => Aione()->theme_options[ 'ec_global_sidebar' ],
					'sidebar_1' => Aione()->theme_options[ 'ec_sidebar' ],
					'sidebar_2' => Aione()->theme_options[ 'ec_sidebar_2' ],
					'position'  => Aione()->theme_options[ 'ec_sidebar_pos' ],
				);
			}
			...
			$sidebars = array(
				...
				'position'  => Aione()->theme_options[ 'search_sidebar_position' ],
			);
		} else {
			$sidebars = array(
				'global'    => Aione()->theme_options[ 'pages_global_sidebar' ],
	...
	public function get_sidebar_settings( $sidebar_options = array() ) {
		// Post options
		$sidebar_1                    = get_post_meta( Aione::c_pageID(), 'sbg_selected_sidebar_replacement', true );
		$sidebar_2                    = get_post_meta( Aione::c_pageID(), 'sbg_selected_sidebar_2_replacement', true );
		$sidebar_position_post_option = strtolower( get_post_meta( Aione::c_pageID(), 'pyre_sidebar_position', true ) );
		$sidebar_position_metadata    = metadata_exists( 'post', Aione::c_pageID(), 'pyre_sidebar_position' );
		...
		if ( Aione()->template->double_sidebars() && 'right' == $sidebar_position ) {
	...
	public function layout_structure_styling( $sidebars ) {
		...
		// Add sidebar class
        add_filter( 'oxo_sidebar_1_class', array( $this, 'sidebar_class' ) );
        add_filter( 'oxo_sidebar_2_class', array( $this, 'sidebar_class' ) );

		// Check for sidebar location and apply styling to the content or sidebar div
		if ( ! Aione()->template->has_sidebar() && ! ( is_page_template( 'side-navigation.php') || is_singular( 'tribe_events' ) ) ) {
			add_filter( 'oxo_content_style', array( $this, 'full_width_content_style' ) );

			if ( is_archive() || is_home() ) {
				add_filter( 'oxo_content_class', array( $this, 'full_width_content_class' ) );
			}
		} elseif ( 'left' == $sidebars['position'] ) {
			add_filter( 'oxo_content_style', array( $this, 'float_right_style' ) );
			add_filter( 'oxo_sidebar_1_style', array( $this, 'float_left_style' ) );
			add_filter( 'oxo_sidebar_1_class', array( $this, 'side_nav_left_class' ) );
		} elseif ( 'right' == $sidebars['position'] ) {
			add_filter( 'oxo_content_style', array( $this, 'float_left_style' ) );
			add_filter( 'oxo_sidebar_1_style', array( $this, 'float_right_style' ) );
			add_filter( 'oxo_sidebar_1_class', array( $this, 'side_nav_right_class' ) );
		}

		// Page has a single sidebar
		if ( Aione()->template->has_sidebar() && ! Aione()->template->double_sidebars() ) {
		} elseif ( Aione()->template->double_sidebars() ) { // Page has double sidebars
			add_filter( 'oxo_content_style', array( $this, 'float_left_style' ) );
			add_filter( 'oxo_sidebar_1_style', array( $this, 'float_left_style' ) );
			add_filter( 'oxo_sidebar_2_style', array( $this, 'float_left_style' ) );

			if( $sidebars['position'] == 'right' ) {
				add_filter( 'oxo_sidebar_2_class', array( $this, 'side_nav_right_class' ) );
			}
		}
	...
	public function join( $filter_id = null, $sanitize = 'esc_attr', $join_separator = ' ' ) {
		...
		$elements = apply_filters( 'oxo_' . $filter_id, array() );
    ...
    public function sidebar_class( $classes ) {
        $classes[] = 'sidebar oxo-widget-area oxo-content-widget-area';
        return $classes;
    }
    ...
    public function get_sidebars_width() {
		...
		if ( Aione()->template->has_sidebar() && ! Aione()->template->double_sidebars() ) {
			if ( get_post_type() == 'tribe_events' ) {
				$sidebar_width = Aione()->settings->get( 'ec_sidebar_width' );
			...
			if ( get_post_type() == 'tribe_events' ) {
				$sidebar_2_1_width = Aione()->settings->get( 'ec_sidebar_2_1_width' );
				...
				$sidebar_2_2_width = Aione()->settings->get( 'ec_sidebar_2_2_width' );
