...
// Don't duplicate me!
	if ( ! class_exists( 'OxoTemplateWoo' ) ) {
		...
		class OxoTemplateWoo {
			...
			function __construct() {
				...
				add_action( 'aione_woocommerce_buttons_on_rollover',  array( $this, 'aione_woocommerce_template_loop_add_to_cart' ), 10 );
				add_action( 'aione_woocommerce_buttons_on_rollover',  array( $this, 'aione_woocommerce_rollover_buttons_linebreak' ), 15 );
				add_action( 'aione_woocommerce_buttons_on_rollover', array( $this, 'show_details_button' ), 20 );
				...
				if ( Aione()->theme_options[ 'woocommerce_product_box_design' ] == 'clean' ) {
					...
					add_action( 'woocommerce_before_shop_loop_item_title', array( $this, 'aione_show_product_loop_outofstock_flash' ), 10 );
					add_action( 'woocommerce_after_shop_loop_item', array( $this, 'before_shop_item_buttons' ), 5 );
					add_action( 'woocommerce_after_shop_loop_item', array( $this, 'aione_woocommerce_template_loop_add_to_cart' ), 10 );
				...
				add_action( 'woocommerce_before_checkout_form', 'aione_top_user_container', 1 );
			...
			function before_container() {
				ob_start();
				Aione()->layout->add_class( 'content_class' );
				$content_class = ob_get_clean();

				ob_start();
				Aione()->layout->add_style( 'content_style' );
			...
			function add_sidebar() {
				do_action( 'oxo_after_content' );
			}

			function aione_show_product_loop_outofstock_flash() {
				global $product;

				if ( ! $product->is_in_stock() ) {
					echo sprintf( '<div class="oxo-out-of-stock"><div class="oxo-position-text">%s</div></div>', __( 'Out of Stock', 'Aione' ) );
				}
			}
			...
			function aione_woocommerce_template_loop_add_to_cart( $args = array() ) {
				global $product;
			...
			function aione_woocommerce_rollover_buttons_linebreak() {
				global $product;
				...
				?>
					<span class="oxo-rollover-linebreak">
						<?php if ( Aione()->theme_options[ 'woocommerce_product_box_design' ] == 'clean' ): ?>/<?php endif; ?>
					</span>			
				<?php
				}
			}
			...
			function show_details_button() {
				...
				echo sprintf( '<a href="%s" class="show_details_button"%s>%s</a>', get_permalink(), $styles, __( 'Details', 'Aione' ) );
			}			
			...
			function change_pagination( $options ) {
				$options['prev_text'] 	= '<span class="page-prev"></span><span class="page-text">' . __('Previous', 'Aione') . '</span>';
				$options['next_text'] 	= '<span class="page-text">' . __('Next', 'Aione') . '</span><span class="page-next"></span>';
				$options['type']		= 'plain';
		...
		} // end OxoTemplateWoo() class

	}
	$oxo_template_woo = new OxoTemplateWoo();
	...
	<div>
	<input type="text" value="' . get_search_query() . '" name="s" class="s" placeholder="' . __( 'Search...', 'Aione' ) . '" />
	...
	if ( ! Aione()->theme_options[ 'woocommerce_aione_ordering' ] ) {
	...
	function aione_woocommerce_catalog_ordering() {
		...
		if ( Aione()->theme_options[ 'woo_items' ] ) {
			$per_page = Aione()->theme_options[ 'woo_items' ];
		} else {
			$per_page = 12;
		}
		...
		switch ( $pob ) {
			case 'date':
				$order_string = __( 'Date', 'Aione' );
				break;
			case 'price':
				$order_string = __( 'Price', 'Aione' );
				break;
			case 'popularity':
				$order_string = __( 'Popularity', 'Aione' );
				break;
			case 'rating':
				$order_string = __( 'Rating', 'Aione' );
				break;
			case 'name':
				$order_string = __( 'Name', 'Aione' );
				break;
			case 'default':
				$order_string = __( 'Default Order', 'Aione' );
				break;
		}
		...
		$html .= '<li>';
		$html .= '<span class="current-li"><span class="current-li-content"><a aria-haspopup="true">' . __( 'Sort by', 'Aione' ) . ' <strong>' . $order_string . '</strong></a></span></span>';
		$html .= '<ul>';
		$html .= '<li class="' . ( ( $pob == 'default' ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_orderby', 'default' ) . '">' . __( 'Sort by', 'Aione' ) . ' <strong>' . __( 'Default Order', 'Aione' ) . '</strong></a></li>';
		$html .= '<li class="' . ( ( $pob == 'name' ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_orderby', 'name' ) . '">' . __( 'Sort by', 'Aione' ) . ' <strong>' . __( 'Name', 'Aione' ) . '</strong></a></li>';
		$html .= '<li class="' . ( ( $pob == 'price' ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_orderby', 'price' ) . '">' . __( 'Sort by', 'Aione' ) . ' <strong>' . __( 'Price', 'Aione' ) . '</strong></a></li>';
		$html .= '<li class="' . ( ( $pob == 'date' ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_orderby', 'date' ) . '">' . __( 'Sort by', 'Aione' ) . ' <strong>' . __( 'Date', 'Aione' ) . '</strong></a></li>';
		$html .= '<li class="' . ( ( $pob == 'popularity' ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_orderby', 'popularity' ) . '">' . __( 'Sort by', 'Aione' ) . ' <strong>' . __( 'Popularity', 'Aione' ) . '</strong></a></li>';
		$html .= '<li class="' . ( ( $pob == 'rating' ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_orderby', 'rating' ) . '">' . __( 'Sort by', 'Aione' ) . ' <strong>' . __( 'Rating', 'Aione' ) . '</strong></a></li>';
		$html .= '</ul>';
		$html .= '</li>';
		$html .= '</ul>';
		...
		$html .= '<ul class="order">';
		if ( $po == 'desc' ):
			$html .= '<li class="desc"><a aria-haspopup="true" href="' . oxo_add_url_parameter( $query_string, 'product_order', 'asc' ) . '"><i class="oxo-icon-arrow-down2 icomoon-up"></i></a></li>';
		endif;
		if ( $po == 'asc' ):
			$html .= '<li class="asc"><a aria-haspopup="true" href="' . oxo_add_url_parameter( $query_string, 'product_order', 'desc' ) . '"><i class="oxo-icon-arrow-down2"></i></a></li>';
		endif;
		$html .= '</ul>';
		...
		$html .= '<ul>';
		$html .= '<li class="' . ( ( $pc == $per_page ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_count', $per_page ) . '">' . __( 'Show', 'Aione' ) . ' <strong>' . $per_page . ' ' . __( 'Products', 'Aione' ) . '</strong></a></li>';
		$html .= '<li class="' . ( ( $pc == $per_page * 2 ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_count', $per_page * 2 ) . '">' . __( 'Show', 'Aione' ) . ' <strong>' . ( $per_page * 2 ) . ' ' . __( 'Products', 'Aione' ) . '</strong></a></li>';
		$html .= '<li class="' . ( ( $pc == $per_page * 3 ) ? 'current' : '' ) . '"><a href="' . oxo_add_url_parameter( $query_string, 'product_count', $per_page * 3 ) . '">' . __( 'Show', 'Aione' ) . ' <strong>' . ( $per_page * 3 ) . ' ' . __( 'Products', 'Aione' ) . '</strong></a></li>';
		$html .= '</ul>';
		$html .= '</li>';
		$html .= '</ul>';

		$woocommerce_toggle_grid_list = Aione()->theme_options[ 'woocommerce_toggle_grid_list' ];
		$product_view = 'grid';
		if ( isset( $_SERVER['QUERY_STRING'] ) ) {
			parse_str( $_SERVER['QUERY_STRING'], $params );
			if( isset($params[ 'product_view' ] ) ){
		...
		if( $woocommerce_toggle_grid_list ){
			$html .= '<ul class="oxo-grid-list-view">';

			if ( $product_view == 'grid' ) {
				$html .= '<li class="oxo-grid-view-li active-view">';
			} else {
				$html .= '<li class="oxo-grid-view-li">';
			}
			$html .= '<a class="oxo-grid-view" aria-haspopup="true" href="' . oxo_add_url_parameter( $query_string, 'product_view', 'grid' ) . '"><i class="oxo-icon-grid icomoon-grid"></i></a>';
			$html .= '</li>';

			if ( $product_view == 'list' ) {
				$html .= '<li class="oxo-list-view-li active-view">';
			} else {
				$html .= '<li class="oxo-list-view-li">';
			}
			$html .= '<a class="oxo-list-view" aria-haspopup="true" href="' . oxo_add_url_parameter( $query_string, 'product_view', 'list' ) . '"><i class="oxo-icon-list icomoon-list"></i></a>';
			$html .= '</li>';
			$html .= '</ul>';
		}
	...
	function aione_woocommerce_get_catalog_ordering_args( $args ) {
		...
		if ( $pob == 'rating' ) {
			...
			add_filter( 'posts_clauses', 'oxo_order_by_rating_post_clauses' );
		}
	...
	/**
	 * oxo_order_by_rating_post_clauses function.
	 *
	 * @access public
	 *
	 * @param array $args
	 ...
	 * @return array
	 */
	function oxo_order_by_rating_post_clauses( $args ) {
		global $wpdb;
	...
	if ( Aione()->theme_options[ 'woocommerce_product_box_design' ] != 'clean' ) {
		...
		function aione_woocommerce_thumbnail() {
			...
			$attachment_image = '';
			if ( ! Aione()->theme_options[ 'woocommerce_disable_crossfade_effect' ] ) {
			...
			echo $thumb_image;
			if ( $in_cart ) {
				echo '<span class="cart-loading"><i class="oxo-icon-check-square-o"></i></span>';
			} else {
				echo '<span class="cart-loading"><i class="oxo-icon-spinner"></i></span>';
			}
			echo '</span>';
		}
	}
	...
	if ( Aione()->theme_options[ 'woocommerce_product_box_design' ] == 'clean' ) {
		...
		function aione_woocommerce_thumbnail() {
			...
			if ( $in_cart ) {
				$classes = 'oxo-item-in-cart';
			}

			$featured_image_markup = aione_render_first_featured_image_markup( $id, $size, $post_permalink, TRUE, FALSE, TRUE, 'disable', 'disable', '', '', 'yes', TRUE );
			echo '<div class="oxo-clean-product-image-wrapper ' . $classes . '">';
				echo $featured_image_markup;
			echo '</div>';
		}
	}

	add_filter( 'wp_nav_menu_items', 'oxo_add_woo_cart_to_widget', 20, 4 );
	function oxo_add_woo_cart_to_widget( $items, $args ) {
		if( class_exists( 'WooCommerce') ) {
			...
			if( $ubermenu == false && $args->container_class == 'oxo-widget-menu' ) {
				$items .= oxo_add_woo_cart_to_widget_html();
			}
		}
	...
	add_filter( 'woocommerce_add_to_cart_fragments', 'aione_woocommerce_header_add_to_cart_fragment' );
	function aione_woocommerce_header_add_to_cart_fragment( $fragments ) {
		global $woocommerce;

		$header_top_cart = aione_nav_woo_cart( 'secondary' );
		$fragments['.oxo-secondary-menu-cart'] = $header_top_cart;

		$header_cart = aione_nav_woo_cart( 'main' );
		$fragments['.oxo-main-menu-cart'] = $header_cart;

		$widget_cart = oxo_add_woo_cart_to_widget_html();
		$fragments['.oxo-widget-cart'] = $widget_cart;
	...
	function aione_woocommerce_after_single_product_summary() {
		...
		$social = '<div class="oxo-clearfix"></div>';
		if ( Aione()->theme_options[ 'woocommerce_social_links' ] ) {
			...
			<li class="facebook">
				<a href="http://www.facebook.com/sharer.php?m2w&s=100&p&#91;url&#93;=' . get_permalink() . '&p&#91;title&#93;=' . wp_strip_all_tags( get_the_title(), true ) . '" target="_blank"' . $nofollow . '>
					<i class="fontawesome-icon medium circle-yes oxo-icon-facebook"></i>
					<div class="oxo-woo-social-share-text"><span>' . __( 'Share On Facebook', 'Aione' ) . '</span></div>
				</a>
			</li>
			<li class="twitter">
				<a href="https://twitter.com/share?text=' . wp_strip_all_tags( get_the_title(), true ) . ' ' . get_permalink() . '" target="_blank"' . $nofollow . '>
					<i class="fontawesome-icon medium circle-yes oxo-icon-twitter"></i>
					<div class="oxo-woo-social-share-text"><span>' . __( 'Tweet This Product', 'Aione' ) . '</span></div>
				</a>
			</li>
			<li class="pinterest">';
			...
			$social .= '<a href="http://pinterest.com/pin/create/button/?url=' . urlencode( get_permalink() ) . '&amp;description=' . urlencode( wp_strip_all_tags( get_the_title(), true ) ) . '&amp;media=' . urlencode( $full_image[0] ) . '" target="_blank"' . $nofollow . '>
					<i class="fontawesome-icon medium circle-yes oxo-icon-pinterest"></i>
					<div class="oxo-woo-social-share-text"><span>' . __( 'Pin This Product', 'Aione' ) . '</span></div>
				</a>
			</li>
			<li class="email">
				<a href="mailto:?subject=' .rawurlencode( html_entity_decode( wp_strip_all_tags( get_the_title(), true ), ENT_COMPAT, 'UTF-8' ) ) . '&amp;body=' . get_permalink() . '" target="_blank"' . $nofollow . '>
					<i class="fontawesome-icon medium circle-yes oxo-icon-mail"></i>
					<div class="oxo-woo-social-share-text"><span>' . __( 'Mail This Product', 'Aione' ) . '</span></div>
				</a>
			</li>
		</ul>';
	...
	function aione_woocommerce_output_related_products() {
		...
		if ( get_post_meta( $post->ID, 'pyre_number_of_related_products', true ) == 'default' ||
			...
			$number_of_columns = Aione()->theme_options[ 'woocommerce_related_columns' ];
		} else {
	...
	add_action( 'woocommerce_after_single_product_summary', 'aione_woocommerce_upsell_display', 10 );
	function aione_woocommerce_upsell_display() {
	...
	function aione_woocommerce_before_cart_table( $args ) {
		...
		if ( $woocommerce->cart->get_cart_contents_count() == 1 ) {
			$html .= '<h2>' . sprintf( __( 'You Have %d Item In Your Cart', 'Aione' ), $woocommerce->cart->get_cart_contents_count() ) . '</h2>';
		} else {
			$html .= '<h2>' . sprintf( __( 'You Have %d Items In Your Cart', 'Aione' ), $woocommerce->cart->get_cart_contents_count() ) . '</h2>';
		}
			...
			<?php echo cart_shipping_calc();
				...
				if ( WC()->cart->coupons_enabled() ) {
					...
					<div class="coupon">
						...
						<input type="submit" class="oxo-button oxo-button-default oxo-button-small button default small"
						       name="apply_coupon" value="<?php _e( 'Apply', 'Aione' ); ?>"/>
	...
	add_action( 'woocommerce_cart_collaterals', 'aione_woocommerce_cross_sell_display', 5 );
	function aione_woocommerce_cross_sell_display() {
		global $product, $woocommerce_loop, $post;
	...
	function aione_woocommerce_checkout_coupon_form( $args ) {
		...
		<form class="woocommerce-content-box full-width checkout_coupon" method="post">
			...
			<h2 class="promo-code-heading oxo-alignleft"><?php _e( 'Have A Promotional Code?', 'Aione' ); ?></h2>

			<div class="coupon-contents oxo-alignright">
				<div class="form-row form-row-first oxo-alignleft coupon-input">
					<input type="text" name="coupon_code" class="input-text"
					       placeholder="<?php _e( 'Coupon code', 'woocommerce' ); ?>" id="coupon_code" value=""/>
				</div>

				<div class="form-row form-row-last oxo-alignleft coupon-button">
					<input type="submit" class="oxo-button button-default button-small button default small"
	...
	function aione_woocommerce_checkout_billing( $args ) {
		...
		if ( ! Aione()->theme_options[ 'woocommerce_one_page_checkout' ] ) {
			...
			<a data-name="<?php echo $data_name; ?>" href="#"
			   class="oxo-button button-default button-medium  button default medium continue-checkout"><?php _e( 'Continue', 'Aione' ); ?></a>
			<div class="clearboth"></div>
	...
	function aione_woocommerce_checkout_shipping( $args ) {
		...
		if ( ! Aione()->theme_options[ 'woocommerce_one_page_checkout' ] ) {
			...
			<a data-name="#order_review" href="#"
			   class="oxo-button button-default button-medium continue-checkout button default medium"><?php _e( 'Continue', 'Aione' ); ?></a>
			<div class="clearboth"></div>
	...
	function aione_enable_order_notes_field() {
		...
		if ( ! Aione()->theme_options[ 'woocommerce_enable_order_notes' ] ) {
			return 0;
		}
	...
	if ( Aione()->theme_options[ 'woocommerce_one_page_checkout' ] ) {
		add_action( 'woocommerce_checkout_after_order_review', 'aione_woocommerce_checkout_after_order_review', 20 );
	}
	function aione_woocommerce_checkout_after_order_review() {
		?></div><?php
	}
...
function aione_top_user_container() {
	global $woocommerce, $current_user;

	echo '<div class="aione_myaccount_user">';
		echo '<span class="myaccount_user_container">';
			echo '<span class="username">';

				if ( $current_user->display_name ) {
					printf( '%s, %s:', __( 'Hello', 'Aione' ), $current_user->display_name );
				} else {
					echo __( 'Hello', 'Aione' );
				}
			...
			if ( Aione()->theme_options[ 'woo_acc_msg_1' ] ) {
				echo '<span class="msg">';
					echo Aione()->theme_options[ 'woo_acc_msg_1' ];
				echo '</span>';
			}

			if ( Aione()->theme_options[ 'woo_acc_msg_2' ] ) {
				echo '<span class="msg">';
					echo Aione()->theme_options[ 'woo_acc_msg_2' ];
				echo '</span>';
			}
			echo '<span class="view-cart">';
				printf( '<a href="%s">%s</a>', get_permalink( get_option( 'woocommerce_cart_page_id' ) ), __( 'View Cart', 'Aione' ) );
			echo '</span>';
		echo '</span>';
	echo '</div>';
}
...
function aione_woocommerce_after_my_account( $args )
{
	...
	<form class="edit-account-form" action="" method="post">
		...
		<p><input type="submit" class="oxo-button button-default button-medium button default medium alignright"
...
/* end order hooks */
/**
* aione_change_product_class - Function to add 'product-list-view' class if the list view is being displayed
...
*/
add_filter( 'post_class', 'aione_change_product_class' );
if(!function_exists( 'aione_change_product_class' )){
	function aione_change_product_class( $classes ){
		if ( isset( $_SERVER[ 'QUERY_STRING' ] ) ) {
			parse_str( $_SERVER[ 'QUERY_STRING' ], $params );
			if( isset( $params[ 'product_view' ] ) ){
				$product_view = $params[ 'product_view' ];
