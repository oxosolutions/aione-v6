...
class Aione_Init {
	...
	public function __construct() {
		...
		add_action( 'aione_before_main_content', array( $this, 'youtube_flash_fix' ) );
	...
	public function load_textdomain(){
		...
		// wp-content/languages/themes/Aione-en_US.mo
		$loaded = load_theme_textdomain( 'Aione', get_template_directory() . '/languages' );

		// wp-content/theme/languages/Aione-en_US.mo
		 if ( ! $loaded ) {
			...
			// wp-content/languages/themes/aione-en_US.mo
		 	if ( ! $loaded ) {
		 		...
		 		// wp-content/languages/Aione-en_US.mo
		 		if ( ! $loaded ) {
					remove_filter( 'theme_locale', array( $this, 'change_locale_lowercase' ) );
					add_filter( 'theme_locale', array( $this, 'change_locale' ), 10, 2 );
					$loaded = load_theme_textdomain( 'Aione', dirname( dirname( get_template_directory() ) ) . '/languages' );

					// wp-content/languages/themes/aione/en_US.mo
					if ( ! $loaded ) {
						remove_filter( 'theme_locale', array( $this, 'change_locale' ) );
						$loaded = load_theme_textdomain( 'Aione', dirname( dirname( get_template_directory() ) ) . '/languages/themes/aione' );
					}
		 		}
		 	}
		}
	...
	/**
	 * Conditionally add theme_support for oxo_builder
	 */
	public function set_builder_status() {

		if ( ! Aione()->theme_options[ 'disable_builder' ] ) {
			add_theme_support( 'oxo_builder' );
		}
	...
	public function set_theme_version() {
		if ( function_exists( 'wp_get_theme' ) ) {
			...
			update_option( 'aione_theme_version', $theme_version );
		}
	...
	public function youtube_flash_fix() {
		echo '<div class="oxo-youtube-flash-fix">&shy;<style type="text/css"> iframe { visibility: hidden; opacity: 0; } </style></div>';
	}

	public function widget_init() {

		register_widget( 'Oxo_Widget_Ad_125_125' );
		register_widget( 'Oxo_Widget_Contact_Info' );
		register_widget( 'Oxo_Widget_Tabs' );
		register_widget( 'Oxo_Widget_Recent_Works' );
		register_widget( 'Oxo_Widget_Tweets' );
		register_widget( 'Oxo_Widget_Flickr' );
		register_widget( 'Oxo_Widget_Social_Links' );
		register_widget( 'Oxo_Widget_Facebook_Page' );
		register_widget( 'Oxo_Widget_Menu' );
	...
	public function set_correct_class_for_menu_items( $menu_items ) {
		...
		foreach ( (array) $menu_items as $key => $menu_item ) {
			...
			if ( 'custom' == $menu_item->object && false !== strpos( $menu_item->url, 'oxo_home_url' ) ) {
				// Replace the oxo_home_url placeholder with the home url
				$menu_item->url = str_replace( 'oxo_home_url', $url, $menu_item->url );
