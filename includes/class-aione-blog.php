...
class Aione_Blog {
    ...
    public function excerpt_length( $length ) {
        ...
        if ( ! is_null( Aione()->theme_options[ 'excerpt_length_blog' ] ) ) {
            $length = Aione()->theme_options[ 'excerpt_length_blog' ];
    	}
    ...
    public function alter_search_loop( $query ) {
        if ( ! is_admin() && $query->is_main_query() && $query->is_search() && Aione()->theme_options[ 'search_results_per_page' ] ) {
            $query->set( 'posts_per_page', Aione()->theme_options[ 'search_results_per_page' ] );
        }
    }
    ...
    public function get_content_stripped_and_excerpted( $excerpt_length, $content ) {
    	...
    	$content = preg_replace_callback( "/$pattern/s", 'aione_extract_shortcode_contents', $content );
    ...
    public function content( $limit, $strip_html ) {
        ...
        $readmore = ( Aione()->theme_options[ 'link_read_more' ] ) ? ' <a href="' . get_permalink( get_the_ID() ) . '">&#91;...&#93;</a>' : ' &#91;...&#93;';
        $readmore = ( Aione()->theme_options[ 'disable_excerpts' ] ) ? '' : $readmore;
        ...
        if ( $raw_content && ! $custom_excerpt ) {
            ...
            if ( 'Characters' == Aione()->theme_options[ 'excerpt_base' ] ) {

                $content  = mb_substr( $content, 0, $limit );
                $content .= ( $limit != 0 && ! Aione()->theme_options[ 'disable_excerpts' ] ) ? $readmore : '';
