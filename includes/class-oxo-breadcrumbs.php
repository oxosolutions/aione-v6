<?php
/**
 * Class Oxo_Breadcrumbs
 *
 * This file does the breadcrumbs handling for the oxo framework
 *
 * @author		OXOSolutions
...
class Oxo_Breadcrumbs {
	...
	public function __construct() {
		...
		$this->options = Aione()->settings->get_all();

		// Setup default array for changeable variables
		$defaults = array(
			 'home_prefix'            => Aione()->theme_options[ 'breacrumb_prefix' ],
			 'separator'              => Aione()->theme_options['breadcrumb_separator'],
			 'show_post_type_archive' => Aione()->theme_options['breadcrumb_show_post_type_archive'],
			 'show_terms'             => Aione()->theme_options['breadcrumb_show_categories'],
			 'home_label'             => __( 'Home', 'Aione' ),
			 'tag_archive_prefix'     => __( 'Tag:', 'Aione' ),
			 'search_prefix'          => __( 'Search:', 'Aione' ),
			 'error_prefix'           => __( '404 - Page not Found', 'Aione' ),
		);

		// Setup a filter for changeable variables and meger it with the defaults
		$args     = apply_filters( 'oxo_breadcrumbs_defaults', $defaults );
	...
	private function wrap_breadcrumbs() {
		$this->html_markup = sprintf( '<div class="oxo-breadcrumbs">%s</div>', $this->html_markup );
	}
	...
	private function get_breadcrumb_prefix() {
		...
		if ( ! is_front_page() ) {
			...
			if ( $this->home_prefix ) {
				$prefix = sprintf( '<span class="oxo-breadcrumb-prefix">%s:</span>', $this->home_prefix );
			}
		}
	...
	private function get_breadcrumb_home() {
		...
		if ( ! is_front_page() ) {
			...
			$home_link = $this->get_single_breadcrumb_markup( Aione()->theme_options[ 'blog_title' ] );
		}
	...
	private function get_post_terms() {
		...
		if ( $this->post->post_type == 'post' ) {
			$taxonomy = 'category';
		// OXOSolutions Portfolio
		} elseif ( $this->post->post_type == 'aione_portfolio' ) {
	...
	private function get_single_breadcrumb_markup( $title, $link = '', $separator = TRUE, $microdata = TRUE ) {
		...
		if ( $separator ) {
			$separator_markup = sprintf( '<span class="oxo-breadcrumb-sep">%s</span>', $this->separator );
		}
