...
class Oxo_Widget_Facebook_Page extends WP_Widget {

	function __construct() {

		$widget_ops = array(
			'classname'   => 'facebook_like',
			'description' => __( 'Adds support for Facebook Page Plugin.', 'Aione' )
		);
		$control_ops = array( 'id_base' => 'facebook-like-widget' );

		parent::__construct( 'facebook-like-widget', 'Aione: Facebook Page Plugin', $widget_ops, $control_ops );
	...
	function widget( $args, $instance ) {
		...
		<?php if ( $page_url ) : ?>
			...
			window.fbAsyncInit = function() {
				oxo_resize_page_widget();
			
				jQuery( window ).resize(function() {
					oxo_resize_page_widget();
				});
				
				function oxo_resize_page_widget() {
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title'        => __( 'Find us on Facebook', 'Aione' ),
			'app_id'       => '',
			'page_url'     => '',
			'width'        => '268',
			'show_faces'   => 'on',
		...
		<h4 style="line-height: 1.6em;"><?php _e( 'IMPORTANT: Facebook has abandoned color scheme option for the page plugin, thus the option is also no longer available in the widget.', 'Aione' ); ?></a></h4>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'page_url' ); ?>"><?php _e( 'Facebook Page URL:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'width' ); ?>"><?php _e( 'Width (has to be between 180 and 500):', 'Aione' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_faces' ); ?>"><?php _e( 'Show Friends Faces', 'Aione' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_stream' ); ?>"><?php _e( 'Show Posts', 'Aione' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_header' ); ?>"><?php _e( 'Show Cover Photo', 'Aione' ); ?></label>
		</p>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'small_header' ); ?>"><?php _e( 'Use Small Header', 'Aione' ); ?></label>
		</p>
	<?php
	}
}
