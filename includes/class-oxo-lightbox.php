...
class Oxo_Lightbox {
    ...
    public function __construct() {
        add_action( 'wp_ajax_oxo_parse_lightbox_url', array( $this, 'parse_url' ) );
        add_action( 'wp_ajax_nopriv_oxo_parse_ligtbox_url', array( $this, 'parse_url' ) );
    }
    ...
    public function parse_url( $url ) {
        ...
        if( $url ){
            if( $this->validate_url( $url ) ) {
                if( false === ( $lightbox_data = get_transient( 'oxo_lightbox_' . md5( $url ) ) ) ) {
