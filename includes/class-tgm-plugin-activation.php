	...
	class TGM_Plugin_Activation {
		...
		public function init() {
			...
			if ( true !== $this->is_tgmpa_complete() ) {
				// Sort the plugins.
				// OXOSolutions edit for Aione theme: no lexicographical ordering of plugin names
				//array_multisort( $this->sort_order, SORT_ASC, $this->plugins );

				// OXOSolutions edit for Aione theme: redirect to Aione plugin page after plugin deactivation
				add_action( 'admin_menu', array( $this, 'deacivate_update_plugin' ), 5 );

				add_action( 'admin_menu', array( $this, 'admin_menu' ) );		
				
				// OXOSolutions edit for Aione theme: delete the dismiss button to make sure OxoCore plugin is updated
		...
		protected function do_plugin_install() {
			...
			if ( ( isset( $_GET['tgmpa-install'] ) && 'install-plugin' === $_GET['tgmpa-install'] ) || ( isset( $_GET['tgmpa-update'] ) && 'update-plugin' === $_GET['tgmpa-update'] ) ) {
				...
				if ( $this->is_tgmpa_complete() ) {
					// OXOSolutions edit for Aione theme: change the return URL to the Oxo Plugin page
					echo '<p><a href="', esc_url( admin_url( 'admin.php?page=aione-plugins' ) ), '" target="_parent">', esc_html( $this->strings['return'] ), '</a></p>';
		...
		public function notices() {
			// OXOSolutions edit for Aione theme: added check if we have an admin user, if not return
			...
			foreach ( $this->plugins as $slug => $plugin ) {
				...
				// OXOSolutions edit for Aione theme: added ! $plugin['required'] to the conditional
			...
			if ( ! empty( $message ) ) {
				...
				// OXOSolutions edit for Aione theme: add class="button-primary" and style="margin-top:1em;" 
