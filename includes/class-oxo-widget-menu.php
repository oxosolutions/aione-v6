...
class Oxo_Widget_Menu extends WP_Widget {

	function __construct() {

		$widget_ops  = array('classname' => 'menu', 'description' => '');
		$control_ops = array('id_base' => 'menu-widget');
		parent::__construct('menu-widget', 'Aione: Horizontal Menu', $widget_ops, $control_ops);
	...
	function widget( $args, $instance ) {
		...
		?>
		<style type="text/css">
		#<?php echo $this->id; ?> > .oxo-widget-menu ul {
		...
		#<?php echo $this->id; ?> > .oxo-widget-menu li {
		...
		#<?php echo $this->id; ?> ul li a {
			...
			padding: 0;
			border: 0;
			color: <?php echo Aione_Sanitize::color( $instance['menu_link_color'] ); ?>;
			font-size: <?php echo Aione_Sanitize::size( $instance['font_size'] ); ?>;
		}
		#<?php echo $this->id; ?> ul li a:after {
			content: '<?php echo $instance['sep_text']; ?>';
			color: <?php echo Aione_Sanitize::color( $instance['menu_link_color'] ); ?>;
			padding-right: <?php echo Aione_Sanitize::size( $instance['menu_padding'] ); ?>;
			padding-left: <?php echo Aione_Sanitize::size( $instance['menu_padding'] ); ?>;
			font-size: <?php echo Aione_Sanitize::size( $instance['font_size'] ); ?>;
		}
		#<?php echo $this->id; ?> ul li a:hover {
			color: <?php echo Aione_Sanitize::color( $instance['menu_link_hover_color'] ); ?>;
		}
		#<?php echo $this->id; ?> ul li:last-child a:after {
			display: none;
		}
		#<?php echo $this->id; ?> ul li .oxo-widget-cart-number {
			background-color: <?php echo Aione_Sanitize::color( $instance['menu_link_hover_color'] ); ?>;
			color: <?php echo Aione_Sanitize::color( $instance['menu_link_color'] ); ?>;
		}
		#<?php echo $this->id; ?> ul li.oxo-active-cart-icon .oxo-widget-cart-icon:after {
			color: <?php echo Aione_Sanitize::color( $instance['menu_link_hover_color'] ); ?>;
		}
		</style>
		<?php
		...
		$nav_menu_args = array(
			...
			'menu'        	  => $nav_menu,
			'depth'		  	  => -1,
			'container_class' => 'oxo-widget-menu'
		);
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'nav_menu' ); ?>"><?php _e( 'Select Menu:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'alignment' ); ?>"><?php _e( 'Alignment:', 'Aione' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'alignment' ); ?>" name="<?php echo $this->get_field_name( 'alignment' ); ?>" class="widefat" style="width:100%;">
				<option value="Left" <?php if ( 'Left' == $instance['alignment']) echo 'selected="selected"'; ?>><?php _e( 'Left', 'Aione' ); ?></option>
				<option value="Center" <?php if ( 'Center' == $instance['alignment']) echo 'selected="selected"'; ?>><?php _e( 'Center', 'Aione' ); ?></option>
				<option value="Right" <?php if ( 'Right' == $instance['alignment']) echo 'selected="selected"'; ?>><?php _e( 'Right', 'Aione' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id( 'menu_padding' ); ?>"><?php _e( 'Menu Padding:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'menu_link_color' ); ?>"><?php _e( 'Menu Link Color:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id('menu_link_hover_color'); ?>"><?php _e( 'Menu Link Hover Color:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'sep_text' ); ?>"><?php _e( 'Separator Text:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'font_size' ); ?>"><?php _e( 'Font Size:', 'Aione' ); ?></label>
