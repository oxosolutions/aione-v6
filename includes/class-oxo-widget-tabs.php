...
class Oxo_Widget_Tabs extends WP_Widget {

	function __construct() {

		$widget_ops  = array( 'classname' => 'oxo-tabs-widget pyre_tabs', 'description' => 'Popular posts, recent post and comments.' );
		$control_ops = array( 'id_base' => 'pyre_tabs-widget' );

		parent::__construct( 'pyre_tabs-widget', 'Aione: Tabs', $widget_ops, $control_ops );
			...
			<div class="tab-hold tabs-wrapper">
				...
				<ul id="tabs" class="tabset tabs">
					...
					<?php if ( 'true' == $show_comments ) : ?>
						<li><a href="#tab-comments"><span class="oxo-icon-bubbles"></span></a></li>
					<?php endif; ?>
					...
					<?php if ( 'true' == $show_popular_posts ) : ?>
						...
						<div id="tab-popular" class="tab tab_content" style="display: none;">
							...
							<ul class="news-list">
									...
									<?php while ( $popular_posts->have_posts() ) : $popular_posts->the_post(); ?>
										<li>
											...
											<div class="post-holder">
												...
												<div class="oxo-meta">
													<?php the_time( Aione()->theme_options[ 'date_format' ] ); ?>
												</div>
											</div>
										</li>
									<?php endwhile; ?>
								...
								<?php else : ?>
									<li><?php _e( 'No posts have been published yet.', 'Aione' ); ?></li>
								<?php endif; ?>
							</ul>
						</div>
									...
									<?php foreach ( $the_comments as $comment ) : ?>
										<li>
											...
											<div class="post-holder">
												...
												<div class="oxo-meta">
													<a class="comment-text-side" href="<?php echo get_permalink( $comment->ID ); ?>#comment-<?php echo $comment->comment_ID; ?>" title="<?php printf( __( '%1$s on %2$s', 'Aione' ), strip_tags( $comment->comment_author ), $comment->post_title ); ?>"><?php echo wp_trim_words( strip_tags( $comment->com_excerpt ), 12 ); ?></a>
												</div>
											</div>
										</li>
									<?php endforeach; ?>
								<?php else : ?>
									<li><?php _e( 'No comments have been published yet.', 'Aione' ); ?></li>
								<?php endif; ?>
							</ul>
						</div>
	...
	function form( $instance ) {
		...
		$defaults = array(
			...
			'show_comments'      => 'on',
			'show_tags'          =>  'on',
			'orderby'            => __( 'Highest Comments', 'Aione' ),
		);

		$instance = wp_parse_args( (array) $instance, $defaults ); ?>

		<p>
			<label for="<?php echo $this->get_field_id( 'orderby' ); ?>"><?php _e( 'Popular Posts Order By:', 'Aione' ); ?></label>
			<select id="<?php echo $this->get_field_id( 'orderby' ); ?>" name="<?php echo $this->get_field_name( 'orderby' ); ?>" class="widefat" style="width:100%;">
				<option <?php if ( __( 'Highest Comments', 'Aione' ) == $instance['orderby'] ) echo 'selected="selected"'; ?>><?php _e( 'Highest Comments', 'Aione' ); ?></option>
				<option <?php if ( __( 'Highest Views', 'Aione' ) == $instance['orderby'] ) echo 'selected="selected"'; ?>><?php _e( 'Highest Views', 'Aione' ); ?></option>
			</select>
		</p>
		<p>
			<label for="<?php echo $this->get_field_id('posts'); ?>"><?php _e( 'Number of popular posts:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'tags' ); ?>"><?php _e( 'Number of recent posts:', 'Aione' ); ?></label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'comments' ); ?>"><?php _e( 'Number of comments:', 'Aione' ); ?></label>
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_popular_posts' ); ?>"><?php _e( 'Show popular posts', 'Aione' ); ?></label>
		</p>
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'show_recent_posts' ); ?>"><?php _e( 'Show recent posts', 'Aione' ); ?></label>
		</p>
		<p>
			<input class="checkbox" type="checkbox" <?php checked( $instance['show_comments'], 'on' ); ?> id="<?php echo $this->get_field_id( 'show_comments' ); ?>" name="<?php echo $this->get_field_name( 'show_comments' ); ?>" />
			<label for="<?php echo $this->get_field_id( 'show_comments' ); ?>"><?php _e( 'Show comments', 'Aione' ); ?></label>
		</p>
		<?php
