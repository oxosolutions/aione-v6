...
class Aione_Dynamic_CSS {
	...
	public function __construct() {
		 ...
		 */
		add_action( 'aione_save_options', array( $this, 'reset_all_transients' ) );
		add_action( 'aione_save_options', array( $this, 'clear_cache' ) );
		add_action( 'aione_save_options', array( $this, 'global_reset_option' ) );
		add_action( 'aione_save_options', array( $this, 'clear_cache' ) );
		/**
		 * Add the CSS
		 */
	...
	public static function set_mode() {
		 ...
		 */
		$mode = ( Aione()->settings->get( 'dynamic_css_compiler' ) ) ? 'file' : 'inline';
		...
		if ( 'file' == $mode && self::needs_update() ) {
			...
			$last_time    = (int) get_option( 'aione_dynamic_css_time' );
	...
	public function enqueue_dynamic_css() {
		...
		if ( 'file' == self::$mode ) {
			 ...
			 */
			wp_enqueue_style( 'aione-dynamic-css', self::file( 'uri' ), array( 'aione-stylesheet' ) );
		}
	...
	public static function can_write() {
		...
		$file_name   = '/aione' . $blog_id . '-' . $page_id . '.css';
		$folder_path = $upload_dir['basedir'] . DIRECTORY_SEPARATOR . 'aione-styles';
	...
	public static function file( $target = 'path' ) {
		 ...
		 */
		$css_uri = trailingslashit( $css_uri_folder ) . 'aione-styles/' . $file_name;
		/**
	...
	public function reset_post_transient( $post_id ) {
		delete_transient( 'aione_dynamic_css_' . $post_id );
	}
	...
	public function add_options() {
		/**
		 * The 'aione_dynamic_css_posts' option will hold an array of posts that have had their css generated.
		 * We can use that to keep track of which pages need their CSS to be recreated and which don't.
		 */
		add_option( 'aione_dynamic_css_time', array(), '', 'yes' );
		/**
		 * The 'aione_dynamic_css_time' option holds the time the file writer was last used.
		 */
		add_option( 'aione_dynamic_css_time', time(), '', 'yes' );
	}

	/**
	 * Update the aione_dynamic_css_posts option when a post is saved.
	 * This adds the current post's ID in the array of IDs that the 'aione_dynamic_css_posts' option has.
	 *
	 * @return  void
	 */
	...
	/**
	 * Update the aione_dynemic_css_posts option when the theme options are saved.
	 * This basically empties the array of page IDS from the 'aione_dynamic_css_posts' option
	 */
	...
	public static function needs_update() {
		...
		/**
		 * Get the 'aione_dynamic_css_posts' option from the DB
		 */
		$option  = get_option( 'aione_dynamic_css_posts', array() );
		/**
		 * Get the current page ID
		...
		/**
		 * If the current page ID exists in the array of pages defined in the 'aione_dynamic_css_posts' option
	...
	public static function update_saved_time() {
		update_option( 'aione_dynamic_css_time', time() );
	}
	...
	public function clear_cache() {
		 ...
		 */
		if ( Aione()->settings->get( 'dynamic_css_compiler' ) && Aione()->settings->get( 'cache_server_ip' ) ) {
	...
	public function clear_varnish_cache( $url ) {
		 ...
		 * Build a varniship
		 */
		if ( Aione()->settings->get( 'cache_server_ip' ) ) {
			$varniship = Aione()->settings->get( 'cache_server_ip' );
	...
	public function add_inline_css() {
		...
		/**
		 * Inline Dynamic CSS
		 * This is here because we need it after all Aione CSS
		 * and W3TC can combine it incorrectly
		 */
		if ( 'inline' == self::$mode ) {
			echo "<style id='aione-stylesheet-inline-css' type='text/css'>" . aione_dynamic_css_cached() . '</style>';
		}
