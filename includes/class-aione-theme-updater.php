...
class Aione_Theme_Updater {
	...
	function __construct( $api_url, $theme_id, $theme_slug ) {
		...
		if( get_option( 'oxo_'. $this->theme_slug .'_update_available' ) ){
	...
	function check_for_update( $transient ) {
		...
		if( $this->api_url == 'http://updates.oxosolutions.com/aione-theme.php' ) {
			...
			$request_args['envato_username'] = $aione_options['tf_username'];
			$request_args['api_key'] = $aione_options['tf_api'];
		}
	...
	function update_notice() {
		...
		if ( $pagenow == 'themes.php' || $pagenow == 'update-core.php' ) {
			...
			<p>' . sprintf( __( 'There is an update available for the %s theme.', 'Aione' ), $theme_name ) . '</p>
			</div>';
		}
	}
}
