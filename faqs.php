...
<div id="content" class="oxo-faqs" <?php Aione()->layout->add_style( 'content_style' ); ?>>
<?php
	...
	if ( ! post_password_required( $post->ID ) ) {
		...
		if ( Aione()->theme_options[ 'faq_filters' ] != 'no' &&
			 $faq_terms
		) {
			...
			echo '<ul class="oxo-filters clearfix">';
				...
				if ( Aione()->theme_options[ 'faq_filters' ] == 'yes' ) {
					echo sprintf( '<li class="oxo-filter oxo-filter-all oxo-active"><a data-filter="*" href="#">%s</a></li>', apply_filters( 'aione_faq_all_filter_name', __( 'All', 'Aione' ) ) );
				...
				foreach ( $faq_terms as $faq_term ) {
					...
					if ( $first_filter ) {
						echo sprintf( '<li class="oxo-filter oxo-active"><a data-filter=".%s" href="#">%s</a></li>', urldecode( $faq_term->slug ), $faq_term->name );

						$first_filter = FALSE;
					} else {
						echo sprintf( '<li class="oxo-filter oxo-hidden"><a data-filter=".%s" href="#">%s</a></li>', urldecode( $faq_term->slug ), $faq_term->name );
					}
				}
		...
		?>
		<div class="oxo-faqs-wrapper">
			<div class="accordian oxo-accordian">
				...
				<?php
					...
					while ( $faq_items->have_posts() ): $faq_items->the_post();
						...
						<div class="oxo-panel panel-default oxo-faq-post <?php echo $post_classes; ?>">
							...
							<div class="panel-heading">
								<h4 class="panel-title toggle">
									<a data-toggle="collapse" class="collapsed" data-parent="#accordian-one" data-target="#collapse-<?php echo get_the_ID(); ?>" href="#collapse-<?php echo get_the_ID(); ?>">
										<div class="oxo-toggle-icon-wrapper">
											<i class="fa-oxo-box"></i>
										</div>
										<div class="oxo-toggle-heading"><?php echo get_the_title(); ?></div>
									</a>
								</h4>
							</div>
							<?php
							echo sprintf( '<div id="collapse-%s" class="panel-collapse collapse">', get_the_ID() );
								echo '<div class="panel-body toggle-content post-content">';
									...
									if ( Aione()->theme_options[ 'faq_featured_image' ] &&
										 has_post_thumbnail()
									) {
