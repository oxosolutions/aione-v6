 ...
 */
function oxo_get_adjacent_post_plus($r, $previous = true ) {
	global $post, $wpdb;
 ...
 */
function oxo_previous_post_link_plus($args = '') {
	return oxo_adjacent_post_link_plus($args, '&laquo; %link', true);
}
 ...
 */
function oxo_next_post_link_plus($args = '') {
	return oxo_adjacent_post_link_plus($args, '%link &raquo;', false);
}
 ...
 */
function oxo_adjacent_post_link_plus($args = '', $format = '%link &raquo;', $previous = true) {
	$defaults = array(
...
//	If Post Types Order plugin is installed, default to sorting on menu_order
	...
	if ( $previous && is_attachment() ) {
		...
		$posts = oxo_get_adjacent_post_plus($r, $previous);
...
//	When num_results > 1, multiple adjacent posts may be returned. Use foreach to display each adjacent post.
	foreach ( $posts as $post ) {
		...
		if ( empty($post->post_title) )
			$title = $previous ? __('Previous Post', 'Aione') : __('Next Post', 'Aione');
