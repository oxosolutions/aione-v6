...
jQuery(document).ready(function($){
	...
	// Activate the Aione admin menu theme option entry when theme options are active
	if ( jQuery( 'a[href="themes.php?page=optionsframework"]' ).hasClass( 'current' ) ) {
		var $aione_menu = jQuery( '#toplevel_page_aione' );

		$aione_menu.addClass( 'wp-has-current-submenu wp-menu-open' );
		$aione_menu.children( 'a' ).addClass( 'wp-has-current-submenu wp-menu-open' );
		$aione_menu.children( '.wp-submenu' ).find( 'li' ).last().addClass( 'current' );
		$aione_menu.children( '.wp-submenu' ).find( 'li' ).last().children().addClass( 'current' );
	}
	...
	$('#of_save').live('click',function() {
		...
		// Aione edit
		$('#of_form :input[name][name!="security"][name!="of_reset"][name!="google_analytics"][name!="space_head"][name!="space_body"][name!="content_above_footer"][name!="custom_css"]').val().replace(/\%22/g, "'");
...
jQuery(document).ready(function($) {
	...
	// Oxo Sorter
	$('.oxo-sortable').each(function() {
		var array = [];
		$(this).find('li').each(function() {
			array.push($(this).attr('id'));
		});
		$(this).closest('.section-oxo_sorter').find('.oxo-sorter-positions').val(array);
	});
	$('.oxo-sortable' ).sortable({
		axis: 'y',
		cursor: 'move',
