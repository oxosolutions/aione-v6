...
if ( ! function_exists( 'aione_elasticslider' ) ) {
	function aione_elasticslider( $term ) {
		...
		if ( ! Aione()->theme_options[ 'status_eslider' ] ) {
			$args				= array(
...
if ( ! function_exists( 'aione_wooslider' ) ) {
	function aione_wooslider( $term ) {
		...
		if ( ! Aione()->theme_options[ 'status_oxo_slider' ] ) {
...
if( ! function_exists( 'aione_get_page_title_bar_contents' ) ) {
	function aione_get_page_title_bar_contents( $post_id, $get_secondary_content = TRUE ) {
		...
		if ( $get_secondary_content ) {
			ob_start();
			if ( oxo_get_option( Aione()->theme_options[ 'page_title_bar_bs' ], 'page_title_breadcrumbs_search_bar', $post_id ) != 'none' ) {
				if ( ( 'Breadcrumbs' == Aione()->theme_options[ 'page_title_bar_bs' ] && in_array( get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ), array( 'breadcrumbs', 'default', '' ) ) ) || 'breadcrumbs' == get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ) ) {
					oxo_breadcrumbs();
				} elseif ( ( 'Search Box' == Aione()->theme_options[ 'page_title_bar_bs' ] && in_array( get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ), array( 'searchbar', 'default', '' ) ) ) || 'searchbar' == get_post_meta( $post_id, 'pyre_page_title_breadcrumbs_search_bar', true ) ) {
					get_search_form();
				}
			}
		...
		if ( '' == get_post_meta( $post_id, 'pyre_page_title_text', true ) || 'default' == get_post_meta( $post_id, 'pyre_page_title_text', true ) ) {
			$page_title_text = Aione()->theme_options[ 'page_title_bar_text' ];
		} else {
		...
		if ( ! $title ) {
			...
			if ( is_home() && get_option( 'show_on_front' ) != 'page' ) {
				$title = Aione()->theme_options[ 'blog_title' ];
			}
		...
		if ( ! $subtitle && is_home() && get_option( 'show_on_front' ) != 'page' ) {
			$subtitle = Aione()->theme_options[ 'blog_subtitle' ];
		}

		if ( ! is_archive() && ! is_search() && ! ( is_home() && ! is_front_page() ) ) {
			if ( 'no' == $page_title_text && ( 'yes' == get_post_meta( $post_id, 'pyre_page_title', true ) || 'yes_without_bar' == get_post_meta( $post_id, 'pyre_page_title', true ) || ( 'hide' != Aione()->theme_options[ 'page_title_bar' ] && 'no' != get_post_meta( $post_id, 'pyre_page_title', true ) ) ) ) {
				$title    = '';
				$subtitle = '';
			}
		} else {
			if ( 'hide' != Aione()->theme_options[ 'page_title_bar' ] && 'no' == $page_title_text ) {
				$title    = '';
				$subtitle = '';
			}
		}
...
if ( ! function_exists( 'aione_current_page_title_bar' ) ) {
	function aione_current_page_title_bar( $post_id  ) {
		...
		if ( ( ! is_archive() || class_exists( 'WooCommerce' ) && is_shop() ) && 
			...
			if ( 'yes' == get_post_meta( $post_id, 'pyre_page_title', true ) || 'yes_without_bar' == get_post_meta( $post_id, 'pyre_page_title', true ) || ( 'hide' != Aione()->theme_options[ 'page_title_bar' ] && 'no' != get_post_meta( $post_id, 'pyre_page_title', true ) ) ) {
			...
			if ( is_home() && Aione()->settings->get( 'blog_show_page_title_bar' ) ) {
				...
				if( 'hide' != Aione()->theme_options[ 'page_title_bar' ] ) {
					aione_page_title_bar( $page_title_bar_contents[0], $page_title_bar_contents[1], $page_title_bar_contents[2] );
				}
			}
		}
...
if ( ! function_exists( 'aione_featured_images_for_pages' ) ) {
	function aione_featured_images_for_pages() {
		if ( ! post_password_required( get_the_ID() ) ) {
			...
			if( ! Aione()->theme_options[ 'featured_images_pages' ] ) {
...
if ( ! function_exists( 'aione_number_of_featured_images' ) ) {
	function aione_number_of_featured_images() {
		...
		for ( $i = 2; $i <= Aione()->theme_options[ 'posts_slideshow_number' ]; $i++ ) {
