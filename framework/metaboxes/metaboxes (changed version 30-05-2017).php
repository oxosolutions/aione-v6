...
class PyreThemeFrameworkMetaboxes {
	...
	public function __construct() {
		...
		$this->data = Aione()->settings->get_all();
	...
	function admin_script_loader() {
		...
		if ( is_admin() && ( in_array( $pagenow, array( 'post-new.php', 'post.php' ) ) ) ) {
			...
			wp_register_script( 'aione_upload', get_template_directory_uri() . '/framework/assets/js/upload.js', array( 'jquery' ), $theme_info->get( 'Version' ) );
	...
	public function add_meta_boxes() {
		...
		$disallowed = array( 'page', 'post', 'attachment', 'aione_portfolio', 'themeoxo_elastic', 'product', 'wpsc-product', 'slide', 'tribe_events' );
		...
		$this->add_meta_box( 'portfolio_options', 'Page Options', 'aione_portfolio' );
		$this->add_meta_box( 'es_options', 'Elastic Slide Options', 'themeoxo_elastic' );
		$this->add_meta_box( 'woocommerce_options', 'Page Options', 'product' );
	...
	public function portfolio_options() {
		$this->render_option_tabs( array( 'portfolio_post', 'page', 'sliders', 'header', 'footer', 'sidebars', 'background',
		'pagetitlebar', 'custom_code', 'seo', 'security' ) );
	}

	public function woocommerce_options() {
		$this->render_option_tabs( array( 'page', 'header', 'footer', 'sidebars', 'sliders', 'background', 'pagetitlebar',
		'custom_code', 'seo', 'security' ), 'product' );
	}
	...
	public function code( $id, $label, $desc = '', $default = '' ) {
		global $post;
		$db_value = get_post_meta( $post->ID, 'pyre_' . $id, true );
		$value = ( metadata_exists( 'post', $post->ID, 'pyre_'. $id ) ) ? $db_value : $default;
		$rows = 15;
		...
		<div class="pyre_metabox_field">
			<div class="pyre_desc code">
				<label for="pyre_<?php echo $id; ?>"><?php echo $label; ?></label>
				<?php if ( $desc ) : ?>
					<p><?php echo $desc; ?></p>
				<?php endif; ?>
			</div>
			<div class="pyre_field code">
				<textarea cols="120" rows="<?php echo $rows; ?>" id="pyre_<?php echo $id; ?>" name="pyre_<?php echo $id; ?>"><?php echo $value; ?></textarea>
			</div>
		</div>
		<?php
