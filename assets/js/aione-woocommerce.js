...
function oxoResizeCrossfadeImages( $parent ) {
...
function oxo_resize_crossfade_images_container( $container ) {
...
function oxo_calc_woocommerce_tabs_layout( $tab_selector ) {
...
jQuery( document ).ready( function() {
	jQuery( 'body' ).on( 'click', '.add_to_cart_button', function(e) {
		...
		$add_to_cart_button.closest( '.product, li' ).find( '.cart-loading' ).find( 'i' ).removeClass( 'oxo-icon-check-square-o' ).addClass( 'oxo-icon-spinner' );
		$add_to_cart_button.closest( '.product, li' ).find( '.cart-loading' ).fadeIn();
		setTimeout( function(){
			$add_to_cart_button.closest( '.product, li' ).find( '.cart-loading' ).find( 'i' ).hide().removeClass( 'oxo-icon-spinner' ).addClass( 'oxo-icon-check-square-o' ).fadeIn();
			jQuery( $add_to_cart_button ).parents( '.oxo-clean-product-image-wrapper, li' ).addClass( 'oxo-item-in-cart' );
		}, 2000 );
	});

	jQuery('li').mouseenter(function() {
		if(jQuery(this).find('.cart-loading').find('i').hasClass('oxo-icon-check-square-o')) {
	...
	if ( jQuery( '.aione_myaccount_user' ).length && jQuery( '.woocommerce-error' ).length && ! jQuery( '.aione-myaccount-nav' ).find( '.active' ).children().hasClass( 'address' ) ) {
		jQuery( '.aione-myaccount-nav' ).find( '.active' ).removeClass( 'active' );
		jQuery( '.aione-myaccount-nav' ).find( '.account' ).parent().addClass( 'active' );
	}
	...
	} else if(aione_myaccount_active.hasClass('address') ) {
		jQuery('.aione-myaccount-data .myaccount_address, .aione-myaccount-data .address').fadeIn();
	} else if(aione_myaccount_active ) {
		jQuery('.aione-myaccount-data .edit-account-form, .aione-myaccount-data .edit-account-heading').fadeIn();
		jQuery('.aione-myaccount-data h2:nth-of-type(1)').hide();
	}

	jQuery('body.rtl .aione-myaccount-data .my_account_orders .order-status').each( function() {
	...
	jQuery('.woocommerce.single-product .related.products > h2' ).each( function() {
		var $related_heading = jQuery( this ).replaceWith( function () {
		    return '<div class="oxo-title title' + $title_sep_class_string + '"><h3 class="' + $headinging_orientation + '">' + jQuery( this ).html() + '</h3><div class="title-sep-container"><div class="title-sep' + $title_sep_class_string + ' "></div></div></div>';
		});
	});
	...
	if ( jQuery( 'body .sidebar' ).css( 'display' ) == 'block' ) {
		oxo_calc_woocommerce_tabs_layout( '.woocommerce-tabs .tabs-horizontal' );
	}

	jQuery('.sidebar .products,.oxo-footer-widget-area .products,#slidingbar-area .products').each(function() {
	...
	jQuery( 'body' ).on( 'click', '.woocommerce-checkout-nav a,.continue-checkout', function(e) {
		var $admin_bar_height = ( jQuery( '#wpadminbar' ).length ) ? jQuery( '#wpadminbar' ).height() : 0,
			$header_div_children = jQuery( '.oxo-header-wrapper').find( 'div' ),
		...
		if ( ! jQuery( '.woocommerce .aione-checkout' ).find( '.woocommerce-invalid' ).is( ':visible' ) ) {
			...
			if ( jQuery( this ).hasClass( 'continue-checkout' ) && jQuery( window ).scrollTop() > 0 ) {
				jQuery( 'html, body' ).animate( {scrollTop: jQuery( '.woocommerce-content-box.aione-checkout' ).offset().top - $admin_bar_height - $sticky_header_height }, 500 );
			}
		}
	...
	jQuery( 'body' ).on( 'click', '.aione-myaccount-nav a', function(e) {
		e.preventDefault();

		jQuery('.aione-myaccount-data h2, .aione-myaccount-data .digital-downloads, .aione-myaccount-data .my_account_orders, .aione-myaccount-data .myaccount_address, .aione-myaccount-data .address, .aione-myaccount-data .edit-account-heading, .aione-myaccount-data .edit-account-form').hide();
