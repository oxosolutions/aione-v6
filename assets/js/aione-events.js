jQuery( document ).ready( function() {
	...
	jQuery( '.oxo-tribe-has-featured-image' ).each(function() {
...
jQuery( window ).load(function() {
	...
	jQuery( '.oxo-events-shortcode' ).each( function() {
		jQuery( this ).find('.oxo-events-meta' ).equalHeights();
	});
...
jQuery( document ).ajaxComplete( function( event, request, settings ) {
	...
	jQuery( this ).find( '.post' ).each(function() {
		jQuery( this ).find( '.oxo-post-slideshow' ).flexslider();
		jQuery( this ).find( '.full-video, .video-shortcode, .wooslider .slide-content' ).fitVids();
	});

	// Fade in new posts when all images are loaded, then relayout isotope
	$posts_container = jQuery( '#tribe-events .oxo-blog-layout-grid' );
