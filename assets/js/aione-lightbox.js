...
$aione_lightbox.activate_lightbox = function( $wrapper ) {
	...
	$wrapper.find( '[data-rel^="prettyPhoto["], [rel^="prettyPhoto["], [data-rel^="iLightbox["], [rel^="iLightbox["]' ).each( function () {
		...
		if ( $image_formats_mask == -13 ) {
			jQuery( this ).addClass( 'oxo-no-lightbox' )
		}

		if ( ! jQuery( this ).hasClass( 'oxo-no-lightbox' ) ) {
	...
	jQuery.each( $groups_arr, function ( $i, $group_name ) {
		...
		if ( jQuery( '[data-rel="' + $group_name + '"], [rel="' + $group_name + '"]' ).length == 1 ) {
			$il_instances.push( jQuery( '[data-rel="' + $group_name + '"], [rel="' + $group_name + '"]' ).iLightBox( $aione_lightbox.prepare_options( $group_name, false ) ) );
		} else {
			$il_instances.push( jQuery( '[data-rel="' + $group_name + '"], [rel="' + $group_name + '"]' ).iLightBox( $aione_lightbox.prepare_options( $group_name ) ) );
		}
	});
	...
	$wrapper.find( "a[rel='prettyPhoto'], a[data-rel='prettyPhoto'], a[rel='iLightbox'], a[data-rel='iLightbox']" ).each( function() {
		$il_instances.push( jQuery( this ).iLightBox( $aione_lightbox.prepare_options( 'single' ) ) );
	});

	// Activate lightbox for single lightbox links
	$wrapper.find( '#lightbox-link, .lightbox-link, .oxo-lightbox-link' ).each( function() {
		$il_instances.push( jQuery( this ).iLightBox( $aione_lightbox.prepare_options( 'single' ) ) );
	});

	//activate lightbox for images within the post content
	if( Boolean( Number( js_local_vars.lightbox_post_images ) ) ) {
		$wrapper.find( '.type-post .post-content a, #posts-container .post .post-content a, .oxo-blog-shortcode .post .post-content a' ).has( 'img' ).each(
			function() {
				...
				if( String( jQuery( this ).attr( 'rel' ) ).indexOf( 'prettyPhoto' ) === -1 && String( jQuery( this ).attr( 'data-rel' ) ).indexOf( 'prettyPhoto' ) === -1 && String( jQuery( this ).attr( 'rel' ) ).indexOf( 'iLightbox' ) === -1 && String( jQuery( this ).attr( 'data-rel' ) ).indexOf( 'iLightbox' ) === -1 && ! jQuery( this ).hasClass( 'oxo-no-lightbox' ) ) {
					jQuery( this ).attr( 'data-caption', jQuery( this ).parent().find('p.wp-caption-text').text() );
					$il_instances.push( jQuery( this ).iLightBox( $aione_lightbox.prepare_options( 'post' ) ) );
				}
			}
		);
	}
...
*/
$aione_lightbox.set_title_and_caption = function(){
...
*/
$aione_lightbox.prepare_options = function( $link_id, $gallery ){
...
*/
$aione_lightbox.refresh_lightbox = function( ) {
...
jQuery(window).load(function() {
	//initialize lightbox
	$aione_lightbox.initialize_lightbox();
});
