jQuery( document ).ready(function() {
	...
	jQuery.fn.oxo_position_menu_dropdown = function( variables ) {
	...
	jQuery.fn.walk_through_menu_items = function() {
		jQuery( this ).oxo_position_menu_dropdown();
	...
	jQuery.fn.oxo_position_megamenu = function( variables ) {
		// side header left handling
		if( jQuery( '.side-header-left' ).length ) {
			return this.each( function() {
				jQuery( this ).children( 'li' ).each( function() {
					var li_item = jQuery( this ),
						megamenu_wrapper = li_item.find( '.oxo-megamenu-wrapper' );
		...
		if( jQuery( '.header-v4' ).length ) {
			reference_elem = jQuery( this ).parent( '.oxo-main-menu' ).parent();
		} else {
			reference_elem = jQuery( this ).parent( '.oxo-main-menu' );
		}

		if( jQuery( this ).parent( '.oxo-main-menu' ).length ) {
				...
				return this.each( function() {
					...
					jQuery( this ).children( 'li' ).each( function() {
						...
						if( megamenu_wrapper.length ) {
							...
							if( jQuery( '.oxo-secondary-main-menu' ).length ) {
								reference_aione_row = jQuery( '.oxo-header-wrapper .oxo-secondary-main-menu .oxo-row' );
							} else {
								reference_aione_row = jQuery( '.oxo-header-wrapper .oxo-row' );
							}
	...
	jQuery.fn.calc_megamenu_responsive_column_widths = function( variables ) {
		jQuery( this ).find( '.oxo-megamenu-menu' ).each( function() {
			var megamenu_holder = jQuery( this ).find( '.oxo-megamenu-holder' ),
				megamenu_holder_full_width = megamenu_holder.data( 'width' ),
				reference_oxo_row = ( jQuery( '.oxo-secondary-main-menu' ).length ) ? jQuery( '.oxo-header-wrapper .oxo-secondary-main-menu .oxo-row' ) : jQuery( '.oxo-header-wrapper .oxo-row' ),
				available_space = reference_oxo_row.width();
			...
			if( available_space < megamenu_holder_full_width ) {
				...
				if( ! megamenu_holder.parents( '.oxo-megamenu-wrapper' ).hasClass( 'oxo-megamenu-fullwidth' ) ) {
					megamenu_holder.find( '.oxo-megamenu-submenu' ).each( function() {
						var submenu = jQuery( this );
	...
	jQuery.fn.position_last_top_menu_item = function( variables ) {
		if( jQuery( this ).children( 'ul' ).length || jQuery( this).children( 'div' ).length ) {
			var last_item = jQuery( this ),
				...
				last_item_child,
				parent_container = jQuery( '.oxo-secondary-header .oxo-row' ),
	...
	// IE8 fixes
	jQuery( '.oxo-main-menu > ul > li:last-child' ).addClass( 'oxo-last-menu-item' );
	if( cssua.ua.ie && cssua.ua.ie.substr(0, 1) == '8' ) {
		jQuery( '.oxo-header-shadow' ).removeClass( 'oxo-header-shadow' );
	}

	// Calculate main menu dropdown submenu position
	if( jQuery.fn.oxo_position_menu_dropdown ) {
		jQuery( '.oxo-dropdown-menu, .oxo-dropdown-menu li' ).mouseenter( function() {
			jQuery( this ).oxo_position_menu_dropdown();
		});

		jQuery( '.oxo-dropdown-menu > ul > li' ).each( function() {
	...
	jQuery( '.oxo-dropdown-menu ' ).mouseenter( function() {
		jQuery( this ).css( 'overflow', 'visible' );
	});
	jQuery( '.oxo-dropdown-menu' ).mouseleave( function() {
		jQuery( this ).css( 'overflow', '' );
	});

	// Search icon show/hide
	jQuery( document ).click( function() {
		jQuery( '.oxo-main-menu-search .oxo-custom-menu-item-contents' ).hide();
		jQuery( '.oxo-main-menu-search' ).removeClass( 'oxo-main-menu-search-open' );
		jQuery( '.oxo-main-menu-search' ).find( 'style' ).remove();
	});

	jQuery( '.oxo-main-menu-search' ).click(function(e) {
		e.stopPropagation();
	});

	jQuery( '.oxo-main-menu-search .oxo-main-menu-icon' ).click(function(e) {
		e.stopPropagation();

		if( jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).css( 'display' ) == 'block' ) {
			jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).hide();
			jQuery( this ).parent().removeClass( 'oxo-main-menu-search-open' );

			jQuery( this ).parent().find( 'style' ).remove();
		} else {
			jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).removeAttr( 'style' );
			jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).show();
			jQuery( this ).parent().addClass( 'oxo-main-menu-search-open' );

			jQuery( this ).parent().append( '<style>.oxo-main-menu{overflow:visible!important;</style>' );

			// position main menu search box on click positioning
			if( js_local_vars.header_position == 'Top' ) {
				if( ! jQuery( 'body.rtl' ).length && jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).offset().left < 0 ) {
					jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).css({
						'left': '0',
						'right': 'auto'
					});
				}

				if( jQuery( 'body.rtl' ).length && jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).offset().left + jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).width()  > jQuery( window ).width() ) {
					jQuery( this ).parent().find( '.oxo-custom-menu-item-contents' ).css({
						'left': 'auto',
						'right': '0'
					});
	...
	if( jQuery.fn.oxo_position_megamenu ) {
		jQuery( '.oxo-main-menu > ul' ).oxo_position_megamenu();

		jQuery( '.oxo-main-menu .oxo-megamenu-menu' ).mouseenter( function() {
			jQuery( this ).parent().oxo_position_megamenu();
		});
	...
	if( jQuery.fn.calc_megamenu_responsive_column_widths ) {
		jQuery( '.oxo-main-menu > ul' ).calc_megamenu_responsive_column_widths();
	...
	jQuery('.oxo-header-wrapper .oxo-secondary-menu > ul > li:last-child').position_last_top_menu_item();

	oxo_reposition_menu_item( '.oxo-main-menu .oxo-main-menu-cart' );
	oxo_reposition_menu_item( '.oxo-secondary-menu .oxo-menu-login-box' );

	function oxo_reposition_menu_item( $menu_item ) {
	...
	jQuery('.oxo-megamenu-menu').mouseenter(function() {
    ...
    jQuery( '.oxo-megamenu-menu' ).mouseover(
		function() {
			jQuery( this ).find( '.oxo-megamenu-widgets-container iframe' ).each(
				function() {
					if( ! iframe_loaded ) {
    ...
    jQuery( '.oxo-megamenu-wrapper iframe' ).mouseover(
		function() {
			jQuery( this ).parents( '.oxo-megamenu-widgets-container').css( 'display', 'block' );
			jQuery( this ).parents( '.oxo-megamenu-wrapper' ).css({ 'opacity': '1', 'visibility': 'visible' });
    	}
    );

    jQuery( '.oxo-megamenu-wrapper iframe' ).mouseout(
		function() {
			jQuery( this ).parents( '.oxo-megamenu-widgets-container').css( 'display', '' );
			jQuery( this ).parents( '.oxo-megamenu-wrapper' ).css({ 'opacity': '', 'visibility': '' });
    	}
    );

	// Position main menu cart dropdown correctly on side-header
	jQuery( '.oxo-navbar-nav .cart' ).find( '.cart-contents' ).position_cart_dropdown();
	...
	jQuery( '.oxo-navbar-nav .search-link' ).click( function() {
		setTimeout( function() {
			jQuery( '.oxo-navbar-nav .search-link' ).parent().find( '.main-nav-search-form' ).position_menu_search_form();
		}, 5 );
	});

	jQuery(window).on( 'resize', function() {
		jQuery( '.oxo-navbar-nav .main-nav-search' ).find( '.main-nav-search-form' ).position_menu_search_form();
	});

	// Set overflow on the main menu correcty to show dropdowns when needed
	jQuery( '.oxo-main-menu' ).mouseover(function() {
		jQuery( this ).css( 'overflow', 'visible' );
	});

	jQuery( '.oxo-main-menu' ).mouseout(function() {
		jQuery( this ).css( 'overflow', '' );
	});
	...
	jQuery( '.oxo-mobile-nav-holder' ).not( '.oxo-mobile-sticky-nav-holder' ).each(function() {
		var $mobile_nav_holder = jQuery( this );
		var $mobile_nav = '';
		var $menu = jQuery( this ).parent().find( '.oxo-main-menu, .oxo-secondary-menu' ).not( '.oxo-sticky-menu' );

		if ( $menu.length ) {
			if ( js_local_vars.mobile_menu_design == 'classic' ) {
				$mobile_nav_holder.append( '<div class="oxo-mobile-selector"><span>' + js_local_vars.dropdown_goto + '</span></div>' );
				jQuery( this ).find( '.oxo-mobile-selector' ).append( '<div class="oxo-selector-down"></div>' );
			}
			...
			$mobile_nav.find( '.oxo-caret, .oxo-menu-login-box .oxo-custom-menu-item-contents, .oxo-menu-cart .oxo-custom-menu-item-contents, .oxo-main-menu-search, li> a > span > .button-icon-divider-left, li > a > span > .button-icon-divider-right' ).remove();

			if ( js_local_vars.mobile_menu_design == 'classic' ) {
				$mobile_nav.find( '.oxo-menu-cart > a' ).html( js_local_vars.mobile_nav_cart );
			} else {
				$mobile_nav.find( '.oxo-main-menu-cart' ).remove();
			}

			$mobile_nav.find( 'li' ).each(function () {
				jQuery( this ).find( '> a > .menu-text' ).removeAttr( 'class' ).addClass( 'menu-text' );

				var classes = 'oxo-mobile-nav-item';

				if( jQuery( this ).hasClass( 'current-menu-item' ) || jQuery( this ).hasClass( 'current-menu-parent' ) || jQuery( this ).hasClass( 'current-menu-ancestor' ) ) {
					classes += ' oxo-mobile-current-nav-item';
				}
			...
			jQuery( this ).find( '.oxo-mobile-selector' ).click(function() {
	...
	jQuery( '.oxo-mobile-sticky-nav-holder' ).each(function() {
		var $mobile_nav_holder = jQuery( this );
		var $mobile_nav = '';
		var $menu = jQuery( this ).parent().find( '.oxo-sticky-menu' );
		...
		$mobile_nav.find( '.oxo-menu-cart, .oxo-menu-login-box, .oxo-main-menu-search' ).remove();
	...
	jQuery( '.oxo-mobile-nav-holder > ul > li' ).each(function() {
		jQuery( this ).find( '.oxo-megamenu-widgets-container' ).remove();

		jQuery( this ).find( '.oxo-megamenu-holder > ul' ).each( function() {
			...
			jQuery( this ).find( '> li' ).each( function() {
				...
				if( jQuery( this ).hasClass( 'current-menu-item' ) || jQuery( this ).hasClass( 'current-menu-parent' ) || jQuery( this ).hasClass( 'current-menu-ancestor' ) || jQuery( this ).hasClass( 'oxo-mobile-current-nav-item' ) ) {
					classes += ' oxo-mobile-current-nav-item';
				}
				jQuery( this ).attr( 'class', classes );

				// Append column titles and title links correctly
				if( ! jQuery( this ).find( '.oxo-megamenu-title a, > a' ).length ) {
					jQuery( this ).find( '.oxo-megamenu-title' ).each( function() {
						if( ! jQuery( this ).children( 'a' ).length ) {
							jQuery( this ).append( '<a href="#">' + jQuery( this ).text() + '</a>' );
						}
					});

					if( ! jQuery( this ).find( '.oxo-megamenu-title' ).length ) {
				...
				jQuery( this ).prepend( jQuery( this ).find( '.oxo-megamenu-title a, > a' ) );

				jQuery( this ).find( '.oxo-megamenu-title' ).remove();
			});
			jQuery( this ).closest( '.oxo-mobile-nav-item' ).append( jQuery( this ) );
		});

		jQuery( this ).find( '.oxo-megamenu-wrapper, .caret, .oxo-megamenu-bullet' ).remove();
	});

	// Mobile Modern Menu
	jQuery( '.oxo-mobile-menu-icons .oxo-icon-bars' ).click(function( e ) {
		e.preventDefault();

		var $wrapper;

		if( jQuery( '.oxo-header-v4').length >= 1 || jQuery( '.oxo-header-v5' ).length >= 1 ) {
			$wrapper = '.oxo-secondary-main-menu';
		...
		if( jQuery( '.oxo-is-sticky' ).length >= 1 && jQuery( '.oxo-mobile-sticky-nav-holder' ).length >= 1 ) {
			jQuery( $wrapper ).find( '.oxo-mobile-sticky-nav-holder' ).slideToggle( 200, 'easeOutQuad' );
		} else {
			jQuery( $wrapper ).find( '.oxo-mobile-nav-holder' ).not( '.oxo-mobile-sticky-nav-holder' ).slideToggle( 200, 'easeOutQuad' );
		}
	});

	jQuery( '.oxo-mobile-menu-icons .oxo-icon-search' ).click(function( e ) {
		e.preventDefault();

		jQuery( '.oxo-secondary-main-menu .oxo-secondary-menu-search, .side-header-wrapper .oxo-secondary-menu-search' ).slideToggle( 200, 'easeOutQuad' );
	});

	/*
	// Collapse mobile menus when on page anchors are clicked
	jQuery( '.oxo-mobile-nav-holder .oxo-mobile-nav-item a:not([href=#])' ).click( function() {
		var $target = jQuery( this.hash );
		if ( $target.length && this.hash.slice( 1 ) !== '' ) {
			if ( jQuery( this ).parents( '.oxo-mobile-menu-design-classic' ).length ) {
				jQuery( this ).parents( '.oxo-menu, .menu' )
					.hide()
					.removeClass( 'mobile-menu-expanded' );
			} else {
				jQuery( this ).parents( '.oxo-mobile-nav-holder' ).hide();
			}
		}
	});
	*/
	...
	if( js_local_vars.submenu_slideout == 1 ) {
		jQuery( '.oxo-mobile-nav-holder > ul li' ).each(function() {
			...
			if( jQuery( this ).find( ' > ul' ).length ) {
				jQuery( this ).prepend( '<span href="#" aria-haspopup="true" class="oxo-open-submenu"></span>' );

				jQuery( this ).find( ' > ul' ).hide();
			}
		});

		jQuery( '.oxo-mobile-nav-holder .oxo-open-submenu' ).click( function(e) {
			e.stopPropagation();
...
jQuery( window ).load(function() {
	// Sticky Header
	if( js_local_vars.header_sticky == '1' && ( jQuery( '.oxo-header-wrapper' ).length >= 1 || jQuery( '#side-header' ).length >= 1 ) ) {
		var $animation_duration = 300;
		if( js_local_vars.sticky_header_shrinkage == '0' ) {
			$animation_duration = 0;
		}
		var $header_parent = jQuery( '.oxo-header' ).parent();
		window.$header_parent_height = $header_parent.outerHeight();
		window.$header_height = jQuery( '.oxo-header' ).outerHeight();
		...
		var $logo = ( jQuery( '.oxo-logo img:visible' ).length ) ? jQuery( '.oxo-logo img:visible' ) : '';
		var $sticky_header_scrolled = false;
		window.$sticky_trigger = jQuery( '.oxo-header' );
		...
		var $standard_logo_height = jQuery( '.oxo-standard-logo' ).height() + parseInt( jQuery( '.oxo-logo' ).data( 'margin-top' ) ) + parseInt( jQuery( '.oxo-logo' ).data( 'margin-bottom' ) );
		window.$initial_desktop_header_height = Math.max( window.$header_height, Math.max( $menu_height + $menu_border_height, $standard_logo_height ) + parseInt( jQuery( '.oxo-header' ).find( '.oxo-row' ).css( 'padding-top' ) ) + parseInt( jQuery( '.oxo-header' ).find( '.oxo-row' ).css( 'padding-bottom' ) ) );
		...
		if( jQuery( '.oxo-header-v4' ).length >= 1 || jQuery( '.oxo-header-v5' ).length >= 1 ) {
			...
			if ( js_local_vars.header_sticky_type2_layout == 'menu_and_logo' || ( Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) && js_local_vars.mobile_menu_design == 'modern' ) ) {
				window.$sticky_trigger = jQuery( '.oxo-sticky-header-wrapper' );
			} else {
				window.$sticky_trigger = jQuery( '.oxo-secondary-main-menu' );
			}
		...
		if ( jQuery( document ).height() - ( window.$initial_desktop_header_height - window.$scrolled_header_height ) < jQuery( window ).height() && js_local_vars.sticky_header_shrinkage == 1 ) {
			...
			jQuery( '.oxo-header-wrapper' ).removeClass( 'oxo-is-sticky' );
		} else {
			window.$sticky_can_be_shrinked = true;
		}
		...
		jQuery( window ).resize(function() {
			...
			if( js_local_vars.header_sticky_tablet != '1' && ( window.$media_query_test_1 ) ) {
				jQuery( '.oxo-header-wrapper, .oxo-header-sticky-height, .oxo-header, .oxo-logo, .oxo-header-wrapper .oxo-main-menu > li a, .oxo-header-wrapper .oxo-secondary-main-menu' ).attr( 'style', '' );
				jQuery( '.oxo-header-wrapper' ).removeClass( 'oxo-is-sticky' );
			...
			if( jQuery(window).width() != resize_width || jQuery(window).height() != resize_height ) { // check for actual resize
				...
				if( jQuery( '#wpadminbar' ).length >= 1 && jQuery( '.oxo-is-sticky' ).length >= 1 ) {
					var $sticky_trigger = jQuery( '.oxo-header' );

					if ( window.$sticky_header_type == 2 ) {
						if ( js_local_vars.header_sticky_type2_layout == 'menu_only' && ( js_local_vars.mobile_menu_design == 'classic' || ! window.$media_query_test_4 ) ) {
							$sticky_trigger = jQuery( '.oxo-secondary-main-menu' );
						} else {
							$sticky_trigger = jQuery( '.oxo-sticky-header-wrapper' );
						}
					}

					// Unset the top value for all candidates
					jQuery( '.oxo-header, .oxo-sticky-header-wrapper, .oxo-secondary-main-menu' ).css( 'top', '' );
				...
				if ( window.$sticky_header_type == 1 ) {
					...
					if ( jQuery( '.oxo-secondary-header' ).length ) {
						window.$sticky_trigger_position = Math.round( jQuery( '.oxo-secondary-header' ).offset().top )  - window.$wp_adminbar_height + jQuery( '.oxo-secondary-header' ).outerHeight();
					// If there is no secondary header, trigger position is 0
					} else {
						window.$sticky_trigger_position = Math.round( jQuery( '.oxo-header' ).offset().top )  - window.$wp_adminbar_height;
					}

					// Desktop mode
					if ( ! Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
						var $logo_height_with_margin = jQuery( '.oxo-logo img:visible' ).outerHeight() + parseInt( js_local_vars.logo_margin_top ) + parseInt( js_local_vars.logo_margin_bottom );
							$main_menu_width = 0;

						// Calculate actual menu width
						jQuery( '.oxo-main-menu > ul > li' ).each( function() {
							$main_menu_width += jQuery( this ).outerWidth();
						});

						// Sticky desktop header
						if ( jQuery( '.oxo-is-sticky' ).length ) {
							if ( $main_menu_width > ( jQuery( '.oxo-header .oxo-row' ).width() - jQuery( '.oxo-logo img:visible' ).outerWidth() ) ) {
								window.$header_height = jQuery( '.oxo-main-menu' ).outerHeight() + $logo_height_with_margin;

								// Headers v2 and v3 have a 1px bottom border
								if ( jQuery( '.oxo-header-v2' ).length || jQuery( '.oxo-header-v3' ).length ) {
							...
							jQuery( '.oxo-header-sticky-height' ).css( 'height', window.$header_height );
							jQuery( '.oxo-header' ).css( 'height', window.$header_height );
					...
					// Mobile mode
					if ( Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
						jQuery( '.oxo-header' ).css( 'height', '' );

						window.$header_height = jQuery( '.oxo-header' ).outerHeight();
						window.$scrolled_header_height = window.$header_height;

						jQuery( '.oxo-header-sticky-height' ).css( 'height', window.$scrolled_header_height );
					}
				}
				...
				if ( window.$sticky_header_type == 2 ) {
					if ( js_local_vars.mobile_menu_design == 'modern' ) {
						...
						if ( ! Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) &&
							 jQuery( '.oxo-is-sticky' ).length &&
							 js_local_vars.header_sticky_type2_layout == 'menu_only'
						) {
							window.$header_parent_height = jQuery( '.oxo-header' ).parent().outerHeight() + jQuery( '.oxo-secondary-main-menu' ).outerHeight();
						} else {
							window.$header_parent_height = jQuery( '.oxo-header' ).parent().outerHeight();
						}
						window.$scrolled_header_height = window.header_parent_height;

						// Desktop Mode
						if ( ! Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
							window.$header_parent_height = jQuery( '.oxo-header' ).outerHeight() + jQuery( '.oxo-secondary-main-menu' ).outerHeight();
							window.$sticky_trigger_position = Math.round( jQuery( '.oxo-header' ).offset().top )  - window.$wp_adminbar_height + jQuery( '.oxo-header' ).outerHeight();

							jQuery( $header_parent ).height( window.$header_parent_height );
							jQuery( '.oxo-header-sticky-height' ).css( 'height', '' );
						}

						// Mobile Mode
						if ( Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
							// Trigger position basis is oxo-secondary-header, if there is a secondary header
							if ( jQuery( '.oxo-secondary-header' ).length ) {
								window.$sticky_trigger_position = Math.round( jQuery( '.oxo-secondary-header' ).offset().top )  - window.$wp_adminbar_height + jQuery( '.oxo-secondary-header' ).outerHeight();
							// If there is no secondary header, trigger position is 0
							} else {
							...
							jQuery( '.oxo-header-sticky-height' ).css( 'height', jQuery( '.oxo-sticky-header-wrapper' ).outerHeight() ).hide();
						}
					}
				...
				if ( window.$sticky_header_type == 3 ) {
					...
					if ( ! Modernizr.mq( 'only screen and (max-width:' + js_local_vars.side_header_break_point + 'px)' ) ) {
						...
						if ( jQuery( '#side-header' ).hasClass( 'oxo-is-sticky' ) ) {
							jQuery( '#side-header' ).css({
								top: ''
							});

							jQuery( '#side-header' ).removeClass( 'oxo-is-sticky' );
						}
					}
				}
				...
				if ( jQuery( document ).height() - ( window.$initial_desktop_header_height - window.$scrolled_header_height ) < jQuery( window ).height() && js_local_vars.sticky_header_shrinkage == 1 ) {
					window.$sticky_can_be_shrinked = false;
					jQuery( '.oxo-header-wrapper' ).removeClass( 'oxo-is-sticky' );
					jQuery( '.oxo-header-sticky-height' ).hide();
					jQuery( '.oxo-header' ).css( 'height', '' );
					...
					jQuery( '.oxo-main-menu > ul > li > a' ).css({
						'height': '',
						'line-height': ''
					});

					jQuery( '.oxo-logo img' ).css( 'height', '' );
				} else {
					window.$sticky_can_be_shrinked = true;

					// Resizing sticky header
					if( jQuery( '.oxo-is-sticky' ).length >= 1 ) {
						if( window.$sticky_header_type == 1 && ! Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
							...
							if( ! Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
								if ( $header_height == window.$initial_desktop_header_height ) {
									...
									jQuery( '.oxo-header-sticky-height' ).show();
									jQuery( '.oxo-header-sticky-height' ).stop(true, true).animate({
										height: window.$scrolled_header_height
									}, { queue: false, duration: $animation_duration, easing: 'easeOutCubic', complete: function() {
										jQuery(this).css( 'overflow', 'visible' );
									} });
								...
								jQuery( '.oxo-header-sticky-height' ).css( 'height', window.$scrolled_header_height ).show();
							}
							...
							if( js_local_vars.sticky_header_shrinkage == '1' && $header_height == window.$initial_desktop_header_height ) {
								...
								jQuery( '.oxo-logo' ).stop(true, true).animate({
									'margin-top': $scrolled_logo_container_margin,
									'margin-bottom': $scrolled_logo_container_margin
								}, { queue: false, duration: $animation_duration, easing: 'easeOutCubic' });

								// Animate Menu Height
								jQuery( '.oxo-main-menu > ul > li > a' ).stop(true, true).animate({
		...
		jQuery( window ).scroll(function() {
			...
			if ( window.$sticky_can_be_shrinked ) {
				...
				if( jQuery( '.oxo-is-sticky' ).length == 0 && jQuery( '.oxo-header, .oxo-secondary-main-menu' ).find( '.oxo-mobile-nav-holder > ul' ).is( ':visible' ) ) {
					window.$sticky_trigger_position = Math.round( jQuery( '.oxo-header, .oxo-sticky-header-wrapper' ).find( '.oxo-mobile-nav-holder:visible' ).offset().top ) - window.$wp_adminbar_height + jQuery( '.oxo-header, .oxo-sticky-header-wrapper' ).find( '.oxo-mobile-nav-holder:visible' ).height();
				}

				// If sticky header is not active, reassign the triggers
				if( window.$sticky_header_type != 3 && jQuery( '.oxo-is-sticky' ).length == 0 && ! jQuery( '.oxo-header, .oxo-secondary-main-menu' ).find( '.oxo-mobile-nav-holder > ul' ).is( ':visible' ) ) {
					window.$sticky_trigger = jQuery( '.oxo-header' );
					window.$sticky_trigger_position = Math.round( window.$sticky_trigger.offset().top )  - window.$wp_adminbar_height;
				...
				if( jQuery( window ).scrollTop() > window.$sticky_trigger_position ) { // sticky header mode
					if( $sticky_header_scrolled == false ) {
						...
						jQuery( '.oxo-header-wrapper' ).addClass( 'oxo-is-sticky' );
						jQuery( window.$sticky_trigger ).css( 'top', $wp_adminbar_height );
						$logo = jQuery( '.oxo-logo img:visible' );

						// Hide all mobile menus
						if( js_local_vars.mobile_menu_design == 'modern' ) {
							jQuery( '.oxo-header, .oxo-secondary-main-menu' ).find( '.oxo-mobile-nav-holder' ).hide();
							jQuery( '.oxo-secondary-main-menu .oxo-main-menu-search .oxo-custom-menu-item-contents' ).hide();
						} else {
							jQuery( '.oxo-header, .oxo-secondary-main-menu' ).find( '.oxo-mobile-nav-holder > ul' ).hide();
						}

						if( js_local_vars.mobile_menu_design == 'modern' ) {
							// hide normal mobile menu if sticky menu is set in sticky header
							if( jQuery( '.oxo-is-sticky' ).length >= 1 && jQuery( '.oxo-mobile-sticky-nav-holder' ).length >= 1 && jQuery( '.oxo-mobile-nav-holder' ).is( ':visible' ) ) {
								jQuery( '.oxo-mobile-nav-holder' ).not( '.oxo-mobile-sticky-nav-holder' ).hide();
							}
						}
						...
						if( window.$sticky_header_type == 1 ) {
							...
							// Add sticky shadow
							setTimeout( function() {
								jQuery( '.oxo-header' ).addClass( 'oxo-sticky-shadow' );
							}, 150 );

							if( js_local_vars.sticky_header_shrinkage == '1' && $header_height == window.$initial_desktop_header_height ) {
								// Animate header padding
								jQuery( window.$sticky_trigger ).find( '.oxo-row' ).stop( true, true ).animate({
									'padding-top': 0,
									'padding-bottom': 0
								}, { queue: false, duration: $animation_duration, easing: 'easeOutCubic' });
					...
					jQuery( '.oxo-header-wrapper' ).removeClass( 'oxo-is-sticky' );
					jQuery( '.oxo-header' ).removeClass( 'oxo-sticky-shadow' );
					$logo = jQuery( '.oxo-logo img:visible' );

					if( js_local_vars.mobile_menu_design == 'modern' ) {
						// hide sticky menu if sticky menu is set in normal header
						if( jQuery( '.oxo-is-sticky' ).length == 0 && jQuery( '.oxo-mobile-sticky-nav-holder' ).length >= 1 && jQuery( '.oxo-mobile-nav-holder' ).is( ':visible' ) ) {
							jQuery( '.oxo-mobile-sticky-nav-holder' ).hide();
						}
					}
					...
					if( window.$sticky_header_type == 1 ) {
						...
						if( ! Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
							...
							jQuery( '.oxo-header-sticky-height' ).hide().css( 'height', window.$header_height + $menu_border_height );
						}
						...
						if( js_local_vars.sticky_header_shrinkage == '1' && $header_height == window.$initial_desktop_header_height ) {
							...
							if( $logo ) {
								$logo.stop( true, true ).animate({
									...
									jQuery( this ).css( 'display', '' );
									jQuery( '.oxo-sticky-logo-1x, .oxo-sticky-logo-2x' ).css( 'height', '' );
								} });
							}

							jQuery( '.oxo-logo' ).stop( true, true ).animate({
								'margin-top': jQuery('.oxo-logo').data('margin-top'),
								'margin-bottom': jQuery('.oxo-logo').data('margin-bottom')
							}, { queue: false, duration: $animation_duration, easing: 'easeOutCubic' });
	...
	function adjust_mobile_menu_settings() {
		...
		if( Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
			jQuery( '.oxo-secondary-menu > ul' ).children( 'li' ).each( function() {
				menu_width += jQuery( this ).outerWidth( true ) + 2;
			});

			if( menu_width > jQuery( window ).width() && jQuery( window ).width() > 318 ) {
				if( ! mobile_menu_sep_added ) {
					jQuery( '.oxo-secondary-menu > ul' ).append( '<div class="oxo-mobile-menu-sep"></div>' );
					jQuery( '.oxo-secondary-menu > ul' ).css( 'position', 'relative' );
					jQuery( '.oxo-mobile-menu-sep' ).css( {
						'position': 'absolute',
						'top': jQuery( '.oxo-secondary-menu > ul > li' ).height() - 1 + 'px',
						'width': '100%',
				...
				jQuery( '.oxo-secondary-menu > ul' ).css( 'position', '' );
				jQuery( '.oxo-secondary-menu > ul' ).find( '.oxo-mobile-menu-sep' ).remove();
...
jQuery( document ).ajaxComplete( function() {
	...
	if( jQuery( '.oxo-is-sticky' ).length >= 1 && window.$sticky_trigger && window.$sticky_header_type != 3 ) {
		...
		if ( window.$sticky_header_type == 2 ) {
			$sticky_trigger = jQuery( '.oxo-secondary-main-menu' );
			$menu_height = $sticky_trigger.find( '.oxo-main-menu > ul > li > a' ).height();
		}
