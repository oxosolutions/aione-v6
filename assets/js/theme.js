var generate_carousel = function() {
	if ( jQuery().carouFredSel ) {
		jQuery( '.oxo-carousel' ).each( function() {
			...
			var $image_size = ( jQuery( this ).attr( 'data-imagesize' ) ) ? jQuery( this ).data( 'imagesize' ) : 'fixed',
				...
				$touch_scroll_class = ( $touch_scroll ) ? ' oxo-carousel-swipe' : '',
				...
				$carousel_height = ( jQuery ( this ).parent().hasClass( 'oxo-image-carousel' ) && $image_size == 'fixed' ) ? '115px' : 'variable',
			...
			jQuery( this ).find( '.oxo-carousel-positioner' ).css( 'margin-left', '-' + $item_margin + 'px' );

			// Add $item_margin as left margin to all items
			jQuery( this ).find( '.oxo-carousel-item' ).css( 'margin-left', $item_margin  + 'px' );

			// Shift the left navigation button $item_margin to the right
			jQuery( this ).find( '.oxo-nav-prev' ).css( 'margin-left', $item_margin + 'px' );
			...
			jQuery( this ).find( 'ul' ).carouFredSel({
				...
				prev: jQuery( this ).find( '.oxo-nav-prev' ),
				next: jQuery( this ).find( '.oxo-nav-next' ),
				onCreate: function( data ) {
					// Make the images visible once the carousel is loaded
					jQuery( this ).find( '.oxo-carousel-item-wrapper' ).css( 'visibility', 'visible' );

					// Make the navigation visible once the carousel is loaded
					jQuery( this ).parents( '.oxo-carousel' ).find( '.oxo-carousel-nav' ).css( 'visibility', 'visible' );

					// Remove overflow: hidden to  make carousel stretch full width
					if ( jQuery( this ).parents( '.oxo-woo-featured-products-slider' ).length ) {
					...
					jQuery( this ).parents( '.oxo-carousel' ).find( '.oxo-nav-next' ).each( function() {
						jQuery( this ).css( 'left', jQuery( this ).parents( '.oxo-carousel' ).find( '.oxo-carousel-wrapper' ).width() - jQuery( this ).width() );
					});

					// Resize the placeholder images correctly in "fixed" picture size carousels
					if ( $image_size == 'fixed' ) {
						jQuery( this ).find( '.oxo-placeholder-image' ).each( function() {
							jQuery( this ).css(	'height', jQuery( this ).parents( '.oxo-carousel-item' ).siblings().first().find( 'img' ).height() );
				...
				wrapper: {
					classname: 'oxo-carousel-wrapper' + $touch_scroll_class,
				}
			});
		});
	}
...
var oxo_reanimate_slider = function( content_container ) {
...
var oxo_calculate_responsive_type_values = function( $custom_sensitivity, $custom_minimum_font_size_factor, $custom_mobile_break_point, $elements ) {
...
function oxo_side_header_scroll() {
	...
	if ( ! $media_query_ipad ) {
		...
		if ( Modernizr.mq( 'only screen and (max-width:' + js_local_vars.side_header_break_point + 'px)' ) ) {
			...
			if ( ! $side_header.hasClass( 'oxo-is-sticky' ) ) {
				$side_header.css({
					'bottom': '',
					'position': ''
				});
...
function add_styles_for_old_ie_versions() {
	...
	// IE10
	if ( cssua.ua.ie == '10.0' ) {
		jQuery( 'head' ).append('<style type="text/css">.layout-boxed-mode .oxo-footer-parallax { left: auto; right: auto; }.oxo-imageframe,.imageframe-align-center{font-size: 0px; line-height: normal;}.oxo-button.button-pill,.oxo-button.button-pill:hover{filter: none;}.oxo-header-shadow:after, body.side-header-left .header-shadow#side-header:before, body.side-header-right .header-shadow#side-header:before{ display: none }.search input,.searchform input {padding-left:10px;} .aione-select-parent .select-arrow,.select-arrow{height:33px;background-color:' + js_local_vars.form_bg_color + '}.search input{padding-left:5px;}header .tagline{margin-top:3px;}.star-rating span:before {letter-spacing: 0;}.aione-select-parent .select-arrow,.gravity-select-parent .select-arrow,.wpcf7-select-parent .select-arrow,.select-arrow{background: #fff;}.star-rating{width: 5.2em;}.star-rating span:before {letter-spacing: 0.1em;}</style>');
	}

	// IE11
	if ( cssua.ua.ie == '11.0' ) {
		jQuery( 'head' ).append('<style type="text/css">.layout-boxed-mode .oxo-footer-parallax { left: auto; right: auto; }</style>');
	}
}
...
function get_sticky_header_height() {
	...
	if ( jQuery( '.oxo-header-v4' ).length || jQuery( '.oxo-header-v5' ).length ) {
		$sticky_header_type = 2;
	}

	// Sticky header is enabled
	if ( js_local_vars.header_sticky == '1' && jQuery( '.oxo-header-wrapper' ).length ) {
		// Desktop mode - headers v1, v2, v3
		if ( $sticky_header_type == 1 ) {
			$sticky_header_height = jQuery( '.oxo-header' ).outerHeight() - 1;
		...
		// Desktop mode - headers v4, v5
			...
			$sticky_header_height = jQuery( '.oxo-secondary-main-menu' ).outerHeight();

			if ( js_local_vars.header_sticky_type2_layout == 'menu_and_logo' ) {
				$sticky_header_height += jQuery( '.oxo-header' ).outerHeight();
			}
		}
		...
		if ( Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)' ) ) {
			...
			if ( js_local_vars.header_sticky_mobile == '1' ) {
				// Classic mobile menu
				if ( jQuery( '.oxo-mobile-menu-design-classic' ).length ) {
					$sticky_header_height = jQuery( '.oxo-secondary-main-menu' ).outerHeight();
				}

				// Modern mobile menu
				if ( jQuery( '.oxo-mobile-menu-design-modern' ).length ) {
					$sticky_header_height = jQuery( '.oxo-header' ).outerHeight();
				}
...
(function( jQuery ) {
	...
	jQuery.fn.init_waypoint = function() {
		if( jQuery().waypoint ) {
			...
			// Counters Box
			jQuery('.oxo-counter-box').not('.oxo-modal .oxo-counter-box').each( function() {
				var $offset = get_waypoint_offset( jQuery( this ) );

				jQuery( this ).waypoint(function() {
					jQuery(this).find('.display-counter').each(function() {
						jQuery(this).oxo_box_counting();
					});
				}, {
					triggerOnce: true,
					offset: $offset
			...
			// Counter Circles
			jQuery('.counter-circle-wrapper').not('.oxo-accordian .counter-circle-wrapper, .oxo-tabs .counter-circle-wrapper, .oxo-modal .counter-circle-wrapper').each( function() {
				var $offset = get_waypoint_offset( jQuery( this ) );

				jQuery( this ).waypoint(function() {
					jQuery(this).oxo_draw_circles();
				}, {
					triggerOnce: true,
					offset: $offset
				});
			...
			jQuery('.counter-circle-wrapper').not('.oxo-modal .counter-circle-wrapper').each( function() {
				var $offset = get_waypoint_offset( jQuery( this ) );
				...
				jQuery( this ).waypoint(function() {
					...
					jQuery(window).on('resize', function() {
						counter_circles.oxo_redraw_circles();
					});
				}, {
					triggerOnce: true,
					offset: $offset
			...
			// Progressbar
			jQuery( '.oxo-progressbar' ).not('.oxo-modal .oxo-progressbar').each( function() {
				var $offset = get_waypoint_offset( jQuery( this ) );

				jQuery( this ).waypoint( function() {
					jQuery(this).oxo_draw_progress();
				}, {
					triggerOnce: true,
					offset: $offset
				});
			...
			jQuery( '.oxo-content-boxes' ).each( function() {
				var $offset = get_waypoint_offset( jQuery( this ) );
				...
				jQuery( this ).waypoint( function() {
					...
					jQuery( this ).find( '.content-box-column' ).each( function() {
						...
						setTimeout( function() {
							jQuery( $element ).find( '.oxo-animated' ).css( 'visibility', 'visible' );

							// this code is executed for each appeared element
							var $animation_type = jQuery( $element ).find( '.oxo-animated' ).data( 'animationtype' ),
								$animation_duration = jQuery( $element ).find( '.oxo-animated' ).data( 'animationduration' );

							jQuery( $element ).find( '.oxo-animated' ).addClass( $animation_type );

							if ( $animation_duration ) {
								jQuery( $element ).find( '.oxo-animated' ).css( '-moz-animation-duration', $animation_duration + 's' );
								jQuery( $element ).find( '.oxo-animated' ).css( '-webkit-animation-duration', $animation_duration + 's' );
								jQuery( $element ).find( '.oxo-animated' ).css( '-ms-animation-duration', $animation_duration + 's' );
								jQuery( $element ).find( '.oxo-animated' ).css( '-o-animation-duration', $animation_duration + 's' );
								jQuery( $element ).find( '.oxo-animated' ).css( 'animation-duration', $animation_duration + 's' );
							}

							if( jQuery( $element ).parents( '.oxo-content-boxes' ).hasClass( 'content-boxes-timeline-horizontal' ) ||
								jQuery( $element ).parents( '.oxo-content-boxes' ).hasClass( 'content-boxes-timeline-vertical' ) ) {
								jQuery( $element ).addClass( 'oxo-appear' );
							}
						}, $delay );

						$delay += parseInt( jQuery( this ).parents( '.oxo-content-boxes' ).attr( 'data-animation-delay' ) );
					});
				}, {
					triggerOnce: true,
					offset: $offset
			...
			// CSS Animations
			jQuery( '.oxo-animated' ).each( function() {
				var $offset = get_waypoint_offset( jQuery( this ) );
				...
				jQuery( this ).waypoint( function() {
					if( ! jQuery( this ).parents( '.oxo-delayed-animation' ).length ) {
	...
	jQuery.fn.oxo_recalculate_carousel = function() {
		jQuery( this ).not( '.oxo-woo-featured-products-slider' ).each( function() {
			...
			setTimeout( function() {
				// Set the position of the right navigation element to make it fit the overall carousel width
				$carousel.find( '.oxo-nav-next' ).each( function() {
					jQuery( this ).css( 'left', $carousel.find( '.oxo-carousel-wrapper' ).width() - jQuery( this ).width() );
				});

				// Resize the placeholder images correctly in "fixed" picture size carousels
				if ( $image_size == 'fixed' ) {
					var $image_heights = $carousel.find( '.oxo-carousel-item' ).map( function () {
							return jQuery( this ).find( 'img' ).height();
						}).get(),
						$needed_height = Math.max.apply( null, $image_heights );

					$carousel.find( '.oxo-placeholder-image' ).each( function() {
						jQuery( this ).css(	'height', $needed_height );
					});
					if ( jQuery( $carousel ).parents( '.oxo-image-carousel' ).length >= 1 ) {
						$carousel.find( '.oxo-image-wrapper' ).each( function() {
							jQuery( this ).css(	'height', $needed_height );
	...
	jQuery.fn.oxo_box_counting = function() {
	...
	jQuery.fn.oxo_draw_circles = function() {
	...
	jQuery.fn.oxo_redraw_circles = function() {
		...
		var $current_size = $counter_circles_wrapper.data( 'currentsize' ),
			...
			$oxo_counters_circle_width = $counter_circles_wrapper.parent().width();

		// Overall container width is smaller than one counter circle; e.g. happens for elements in column shortcodes
		if ( $oxo_counters_circle_width < $counter_circles_wrapper.data( 'currentsize' ) ) {

			$counter_circles_wrapper.css({
				'width': $oxo_counters_circle_width,
				'height': $oxo_counters_circle_width,
				'line-height': $oxo_counters_circle_width + 'px'
			});
			$counter_circles_wrapper.find( '.oxo-counter-circle' ).each( function() {
				jQuery( this ).css({
					'width': $oxo_counters_circle_width,
					'height': $oxo_counters_circle_width,
					'line-height': $oxo_counters_circle_width + 'px',
					'font-size': 50 * $oxo_counters_circle_width / 220
				});
				jQuery( this ).data( 'size', $oxo_counters_circle_width );
				jQuery( this ).data( 'strokesize', $oxo_counters_circle_width / 220 * 11 );
				jQuery( this ).data( 'animate', false );
				jQuery( this ).attr( 'data-size', $oxo_counters_circle_width );
				jQuery( this ).attr( 'data-strokesize', $oxo_counters_circle_width / 220 * 11 );
				jQuery( this ).children( 'canvas' ).remove();
				jQuery( this ).removeData( 'easyPieChart' );
			});

			$counter_circles_wrapper.oxo_draw_circles();
		} else {
			$counter_circles_wrapper.css({
				'width': $original_size,
	...
	jQuery.fn.oxo_draw_progress = function() {
	...
	jQuery.fn.oxo_calc_flip_boxes_height = function() {
	...
	// oxo scroller plugin to change css while scrolling
	jQuery.fn.oxo_scroller = function( options ) {
	...
	jQuery.fn.oxo_switch_tab_on_link_click = function( $custom_id ) {
	...
	jQuery.fn.equalHeights = function( $min_height, $max_height ) {
		if ( Modernizr.mq( 'only screen and (min-width: ' + js_local_vars.content_break_point + 'px)' ) || Modernizr.mq( 'only screen and (min-device-width: 768px) and (max-device-width: 1024px) and (orientation: portrait)' ) ) {
			...
			this.each( function() {
				...
				jQuery( this ).find( '.oxo-column-table' ).css('height', 'auto' );
			...
			return this.each( function() {
                ...
                if ( jQuery( this ).children( '.oxo-column-table' ).length ) {
                    $new_height = $tallest - ( jQuery( this ).outerHeight() - jQuery( this ).height() );
                }

				jQuery( this ).css( 'min-height', $new_height );
				jQuery( this ).find( '.oxo-column-table' ).height( $new_height );
			});
		} else {
			return this.each( function() {
				jQuery( this ).css( 'min-height', '' );
				jQuery( this ).find( '.oxo-column-table' ).css( 'height', '' );
			});
		}
	};

	// Set the bg image dimensions of an empty column as data attributes
	jQuery.fn.oxo_set_bg_img_dims = function() {
		jQuery( this ).each( function() {
			if ( ( jQuery.trim( jQuery( this ).html() ) == '<div class="oxo-clearfix"></div>' || jQuery.trim( jQuery( this ).html() ) == '<div class="oxo-column-table" style="height: 0px;"><div class="oxo-column-tablecell"><div class="oxo-clearfix"></div></div></div>' ) && jQuery( this ).data( 'bg-url' ) ) {
	...
	jQuery.fn.oxo_calculate_empty_column_height = function() {

		jQuery( this ).each( function() {
			if ( ( jQuery( this ).parents( '.oxo-equal-height-columns' ).length && ( Modernizr.mq( 'only screen and (max-width: ' + js_local_vars.content_break_point + 'px)' ) || jQuery( this ).data( 'empty-column' ) == true ) ) || ! jQuery( this ).parents( '.oxo-equal-height-columns' ).length ) {
				if ( jQuery.trim( jQuery( this ).html() ) == '<div class="oxo-clearfix"></div>' ) {
	...
	jQuery.fn.reinitialize_google_map = function() {
		var oxo_map_object = jQuery( this ).data( 'plugin_oxo_maps' );

		if ( oxo_map_object ) {
			var	map = oxo_map_object.map,
				center = map.getCenter(),
				markers = oxo_map_object.markers;
	...
	// Initialize oxo filters and corresponding posts
	jQuery.fn.oxo_filters_initialization = function( $posts ) {
		...
		if ( jQuery( this ).length ) {
			...
			var $filters = jQuery( this ).find( '.oxo-filter' ),
				$filter_active = jQuery( this ).find( '.oxo-active' ),
					...
					if ( $posts ) {
						...
						$posts.each( function() {
							var $post = jQuery( this ),
								$post_gallery_name = $post.find( '.oxo-rollover-gallery' ).data( 'rel' );

							// If a post belongs to an invisible filter, fade it in
							if ( $post.hasClass( $filter_name.substr( 1 ) ) ) {
								if ( $filter.hasClass( 'oxo-hidden' ) ) {
									$filter.removeClass( 'oxo-hidden' );
								}
							}
							...
							if ( $filter_active.length && $post.hasClass( $filter_active ) ) {
								...
								$post.find( '.oxo-rollover-gallery' ).attr( 'data-rel', $post_gallery_name.replace( 'gallery', $filter_active ) );
							}
						});
					}
				});
			...
			if ( $filter_active_data_slug.length ) {
				...
				$il_instances.push( jQuery( '[data-rel="iLightbox[' + $filter_active + ']"], [rel="iLightbox[' + $filter_active + ']"]' ).iLightBox( $aione_lightbox.prepare_options( 'iLightbox[' + $filter_active + ']' ) ) );

				// Refresh the lightbox
				$aione_lightbox.refresh_lightbox();
	...
	jQuery.fn.oxo_footer_parallax = function() {
		...
		setTimeout( function() {
			...
			if ( $footer.outerHeight() < jQuery( window ).height() && $wrapper_height > jQuery( window ).height() && ( js_local_vars.header_position == 'Top' || ( js_local_vars.header_position != 'Top' && jQuery( window ).height() > jQuery( '.side-header-wrapper' ).height() ) ) && ( Modernizr.mq( 'only screen and (min-width:'  + parseInt( js_local_vars.side_header_break_point ) +  'px)' ) && ! Modernizr.mq( 'only screen and (min-device-width: 768px) and (max-device-width: 1024px) and (orientation: portrait)' ) && ! Modernizr.mq( 'only screen and (min-device-width: 768px) and (max-device-width: 1024px) and (orientation: landscape)' ) ) ) {
				...
				if( jQuery('.tfs-slider').length >= 1 && jQuery('.tfs-slider').data('parallax') == 1 && $footer.hasClass('oxo-footer-parallax') ) {
	...
	jQuery.fn.oxo_countdown = function() {
	...
	jQuery.fn.oxo_deactivate_mobile_image_hovers = function() {
		if ( js_local_vars.disable_mobile_image_hovers == 1 ) {
			if ( Modernizr.mq( 'only screen and (max-width:' + js_local_vars.side_header_break_point + 'px)' ) ) {
				jQuery( this ).removeClass( 'oxo-image-hovers' );
			} else {
				jQuery( this ).addClass( 'oxo-image-hovers' );
			}
		}
	};

	// Add/remove the mobile title class, depending on available space and title length
	jQuery.fn.oxo_responsive_title_shortcode = function() {
		jQuery( this ).each( function() {
			...
			if ( $title_min_width == 0 && $wrapping_parent_width == 0 ) {
				$title_wrapper.removeClass( 'oxo-border-below-title' );
			} else if ( $title_min_width + 100 >= $wrapping_parent_width ) {
				$title_wrapper.addClass( 'oxo-border-below-title' );
...
jQuery( window ).load( function() { // start window_load_1
	...
	// Counters Box - Modals
	jQuery('.oxo-modal .oxo-counter-box').each(function() {
	...
	jQuery('.oxo-accordian .counter-circle-wrapper, .oxo-tabs .counter-circle-wrapper, .oxo-modal .counter-circle-wrapper').each(function() {
		jQuery(this).appear(function() {
			jQuery(this).oxo_draw_circles();
		});
	});
	...
	// Progressbar - Modals
	jQuery('.oxo-modal .oxo-progressbar').each(function() {
		jQuery(this).appear(function() {
			jQuery(this).oxo_draw_progress();
		});
	});
	...
	jQuery( '.flip-box-inner-wrapper' ).each( function() {
		jQuery( this ).oxo_calc_flip_boxes_height();
	});
	...
	if ( jQuery().cycle ) {
		...
		jQuery( '.oxo-testimonials .reviews' ).each( function() {
	...
	// Toggles
	jQuery( document ).on('click', '.oxo-accordian .panel-title a', function ( e ) {
		e.preventDefault();
		...
		if( clicked_toggle.hasClass( 'active' ) ) {
			clicked_toggle.parents( '.oxo-accordian ').find( '.panel-title a' ).removeClass( 'active' );
		} else {
			clicked_toggle.parents( '.oxo-accordian ').find( '.panel-title a' ).removeClass( 'active' );
			...
			setTimeout( function(){
				...
				if( toggle_content_to_activate.find( '.oxo-carousel' ).length ) {
					generate_carousel();
				}

				toggle_content_to_activate.find( '.oxo-portfolio' ).each( function() {
					jQuery( this ).find( '.oxo-portfolio-wrapper' ).isotope();
				});
				...
				toggle_content_to_activate.find( '.oxo-blog-shortcode' ).each( function() {
					var columns = 2;
					for( i = 1; i < 7; i++ ) {
						if( jQuery( this ).find( '.oxo-blog-layout-grid' ).hasClass( 'oxo-blog-layout-grid-' + i ) ) {
							columns = i;
						}
					}

					var grid_width = Math.floor( 100 / columns * 100 ) / 100  + '%';
					jQuery( this ).find( '.oxo-blog-layout-grid' ).find( '.oxo-post-grid' ).css( 'width', grid_width );

					jQuery( this ).find( '.oxo-blog-layout-grid' ).isotope();
	...
	jQuery( '.oxo-modal' ).each( function() {
		jQuery( '#wrapper' ).append( jQuery( this ) );
	});

	jQuery( '.oxo-modal' ).bind('hidden.bs.modal', function () {
		jQuery( 'html' ).css( 'overflow', '' );
	});

	jQuery( '.oxo-modal' ).bind('show.bs.modal', function () {
		...
		setTimeout( function(){
			...
			modal_window.find( '.oxo-youtube' ).find( 'iframe' ).each( function(i) {
				if ( jQuery( this ).parents( '.oxo-video' ).data( 'autoplay' ) == true ) {
					jQuery( this ).parents( '.oxo-video' ).data( 'autoplay', 'false' );
			...
			modal_window.find( '.oxo-vimeo' ).find( 'iframe' ).each( function(i) {
				if ( jQuery( this ).parents( '.oxo-video' ).data( 'autoplay' ) == true ) {
					jQuery( this ).parents( '.oxo-video' ).data( 'autoplay', 'false' );
			...
			if( modal_window.find( '.oxo-carousel' ).length ) {
				generate_carousel();
			}

			// Reinitialize blog shortcode isotope grid
			modal_window.find( '.oxo-blog-shortcode' ).each( function() {
				var columns = 2;
				for( i = 1; i < 7; i++ ) {
					if( jQuery( this ).find( '.oxo-blog-layout-grid' ).hasClass( 'oxo-blog-layout-grid-' + i ) ) {
						columns = i;
			...
			modal_window.find( '.oxo-portfolio' ).each( function() {
				jQuery( this ).find( '.oxo-portfolio-wrapper' ).isotope();
			});

			// Reinitialize testimonial height; only needed for hidden wrappers
			if ( $slidingbar.find( '.oxo-testimonials' ).length ) {
				var $active_testimonial = $slidingbar.find( '.oxo-testimonials .reviews' ).children( '.active-testimonial' );

				$slidingbar.find( '.oxo-testimonials .reviews' ).height( $active_testimonial.height() );
			}
	...
	if( jQuery( '#sliders-container .tfs-slider' ).data( 'parallax' ) == 1 ) {
		jQuery( '.oxo-modal' ).css( 'top', jQuery( '.header-wrapper' ).height() );
	}
    ...
    jQuery( '.oxo-modal' ).each(function() {
        jQuery( this ).on( 'hide.bs.modal', function() {
			...
			// Vimeo
			jQuery( this ).find('.oxo-vimeo iframe').each(function(i) {
				$f(this).api('pause');
			});
		});
	});
	...
	jQuery( '.oxo-modal-text-link' ).click( function( e ) {
		e.preventDefault();
	});

	if(cssua.ua.mobile || cssua.ua.tablet_pc) {
		jQuery('.oxo-popover, .oxo-tooltip').each(function() {
	...
	if( jQuery( '.vertical-tabs' ).length ) {
		jQuery( '.vertical-tabs .tab-content .tab-pane' ).each( function() {
			...
			if( jQuery ( this ).find( '.video-shortcode' ).length ) {
				var video_width = parseInt( jQuery ( this ).find( '.oxo-video' ).css( 'max-width' ).replace( 'px', '' ) );
				jQuery ( this ).css({
					'float': 'none',
	...
	jQuery( '[data-toggle="tooltip"]' ).each( function() {
		if ( jQuery( this ).parents( '.oxo-header-wrapper' ).length ) {
			$container = '.oxo-header-wrapper';
	...
	jQuery( '.oxo-tooltip' ).hover( function(){
	 ...
	 jQuery( '.oxo-tooltip' ).click( function(){
	...
	jQuery( '.tribe_events_filters_close_filters, .tribe_events_filters_show_filters' ).on( 'click', function() {
		...
		setTimeout( function() {
			jQuery( tribe_events ).parents( '#tribe-events-content-wrapper' ).find( '.oxo-blog-layout-grid' ).isotope();
		});
	});
	...
	jQuery( '.oxo-fullwidth.oxo-equal-height-columns' ).each( function() {
		jQuery( this ).find( '.oxo-layout-column .oxo-column-wrapper' ).equalHeights();
	});

	jQuery( '.oxo-layout-column .oxo-column-wrapper' ).oxo_set_bg_img_dims();
	jQuery( '.oxo-layout-column .oxo-column-wrapper' ).oxo_calculate_empty_column_height();
	...
	jQuery( '.oxo-widget-cart-number' ).css( 'padding', 0 );
	var cart_icon_height = jQuery( '.oxo-widget-cart-number' ).height();
	jQuery( '.oxo-widget-cart-number' ).css( 'width', cart_icon_height + 10 );
	jQuery( '.oxo-widget-cart-number' ).css( 'height', cart_icon_height + 10 );
	jQuery( '.oxo-widget-cart-number' ).css( 'line-height', ( cart_icon_height + 10 ) + 'px' );
}); // end window_load_1

jQuery( document ).ajaxComplete( function() {
	jQuery( '.wpcf7-response-output' ).each( function() {
		if ( jQuery( this ).hasClass( 'wpcf7-validation-errors' ) && ! jQuery( this ).find( '.alert-icon' ).length ) {
			jQuery( this ).addClass( 'oxo-alert' );
		...
		jQuery('.wpcf7-response-output.oxo-alert .close').click(function(e) {
			e.preventDefault();
...
jQuery( document ).ready(function($) { // start document_ready_1
	...
	jQuery( 'body').oxo_deactivate_mobile_image_hovers();
	jQuery(window).on( 'resize', function() {
		jQuery( 'body').oxo_deactivate_mobile_image_hovers();
	});

	// Setup the countdown shortcodes
	jQuery( '.oxo-countdown-counter-wrapper' ).each( function() {
		$countdown_id = jQuery( this ).attr( 'id' );
		jQuery( '#' + $countdown_id ).oxo_countdown();
	});


	// Make the side header scrolling happen
	jQuery( window ).on( 'scroll', oxo_side_header_scroll );
	jQuery( window ).on( 'resize', oxo_side_header_scroll );
	...
	if ( js_local_vars.typography_responsive == 1 ) {
		...
		oxo_calculate_responsive_type_values( js_local_vars.typography_sensitivity, js_local_vars.typography_factor, 800, 'h1, h2, h3, h4, h5, h6' );
	}

	jQuery( '.tfs-slider' ).each(function() {
		oxo_calculate_responsive_type_values( jQuery( this ).data( 'typo_sensitivity' ), jQuery( this ).data( 'typo_factor' ), 800, '.tfs-slider h2, .tfs-slider h3' );
	});

	// Carousel resize
	jQuery(window).on( 'resize', function() {
		jQuery( '.oxo-carousel' ).oxo_recalculate_carousel();
	});

	// Enable autoplaying videos when not in a modal
	jQuery( '.oxo-video' ).each( function() {
		if ( ! jQuery( this ).parents( '.oxo-modal' ).length && jQuery( this ).data( 'autoplay' ) == 1 ) {
			jQuery( this ).find( 'iframe' ).each(function(i) {
	...
	jQuery( '.oxo-footer-parallax' ).oxo_footer_parallax();
	...
	jQuery( '.oxo-footer .oxo-footer-widget-area .oxo-column' ).each(
		function() {
	...
	if ( jQuery( '.comment-respond .comment-reply-title' ).length ) {
		...
		jQuery( '.comment-respond .comment-reply-title' ).wrap( '<div class="oxo-title title oxo-title-size-three' + $title_sep_class_string + '"></div>' );
	...
	jQuery('.oxo-flip-box').mouseover(function() {
		jQuery(this).addClass('hover');
	});

	jQuery('.oxo-flip-box').mouseout(function() {
		jQuery(this).removeClass('hover');
	});

	jQuery('.oxo-accordian .panel-title a').click(function(e) {
		e.preventDefault();
	});
	...
	jQuery( '.sb-toggle' ).click( function(){
		...
		if ( slidingbar_state === 0 ) {
			...
			// Reinitialize carousels
			if( $slidingbar.find( '.oxo-carousel' ).length ) {
				generate_carousel();
			}

			jQuery( '#slidingbar' ).find( '.oxo-carousel' ).oxo_recalculate_carousel();
	...
	if( ! jQuery( '.oxo-social-links-footer' ).find( '.oxo-social-networks' ).children().length ) {
		jQuery( '.oxo-social-links-footer' ).hide();
		jQuery( '.oxo-footer-copyright-area .oxo-copyright-notice' ).css( 'padding-bottom', '0' );
	}
	...
	jQuery('.sh-mobile-nav-holder').append(jQuery('.nav-holder .oxo-navbar-nav').clone());
	jQuery('.sh-mobile-nav-holder .oxo-navbar-nav').attr("id","mobile-nav");
	jQuery('.sh-mobile-nav-holder ul#mobile-nav').removeClass('oxo-navbar-nav');
	...
	/*
	jQuery( '.oxo-menu a:not([href=#], .oxo-megamenu-widgets-container a, .search-link), .oxo-mobile-nav-item a:not([href=#], .search-link), .oxo-button:not([href=#], input, button), .oxo-one-page-text-link:not([href=#])' ).click( function() {
			...
			if ( this.hash ) {
				...
				if ( $target.length && this.hash.slice( 1 ) !== '' ) {
					...
					if ( $target.hasClass( 'tab-link' ) ) {
						jQuery( '.oxo-tabs' ).oxo_switch_tab_on_link_click();
					}
	...
	*/
	...
	jQuery('.oxo-menu a[href*="#"]:not([href^="#"]), .oxo-one-page-text-link[href*="#"]:not([href^="#"])' ).on( 'click', function( e ) {
		// Current path
	...
	if ('ontouchstart' in document.documentElement || navigator.msMaxTouchPoints) {
		jQuery('.oxo-main-menu li.menu-item-has-children > a, .oxo-secondary-menu li.menu-item-has-children > a, .order-dropdown > li .current-li').on("click", function (e) {
			...
			if (link.hasClass('hover')) {
				...
				link.addClass("hover");
				jQuery('.oxo-main-menu li.menu-item-has-children > a, .oxo-secondary-menu li.menu-item-has-children > a, .order-dropdown > li .current-li').not(this).removeClass("hover");
				return false;
			}
		});


		jQuery('.sub-menu li, .oxo-mobile-nav-item li').not('li.menu-item-has-children').on("click", function (e) {
	...
	jQuery( '.oxo-main-menu li.menu-item-has-children > a, .oxo-secondary-menu li.menu-item-has-children > a, .side-nav li.page_item_has_children > a' ).each( function() {
	...
	jQuery( '.nav-tabs li' ).click( function(e) {
		...
		clicked_tab.parents( '.oxo-tabs' ).find( '.nav li' ).removeClass( 'active' );

		if ( clicked_tab.parents( '.oxo-tabs' ).find( tab_content_to_activate ).find( '.oxo-woo-slider' ).length ) {
			var $nav_tabs_height = 0;
			if ( clicked_tab.parents( '.oxo-tabs' ).hasClass( 'horizontal-tabs' ) ) {
				$nav_tabs_height = clicked_tab.parents( '.oxo-tabs' ).find( '.nav' ).height();
			}
			clicked_tab.parents( '.oxo-tabs' ).height( clicked_tab.parents( '.oxo-tabs' ).find( '.tab-content' ).outerHeight( true ) + $nav_tabs_height );
		}

		setTimeout( function(){
			// Google maps
			clicked_tab.parents( '.oxo-tabs' ).find( tab_content_to_activate ).find( '.shortcode-map' ).each(function() {
				jQuery( this ).reinitialize_google_map();
			});

			// Image Carousels
			if( clicked_tab.parents( '.oxo-tabs' ).find( tab_content_to_activate ).find( '.oxo-carousel' ).length ) {
				generate_carousel();
			}

			// Portfolio
			clicked_tab.parents( '.oxo-tabs' ).find( tab_content_to_activate ).find( '.oxo-portfolio' ).each( function() {
				var $portfolio_wrapper = jQuery( this ).find( '.oxo-portfolio-wrapper' ),
			...
			// Flip Boxes
			clicked_tab.parents( '.oxo-tabs' ).find( tab_content_to_activate ).find( '.flip-box-inner-wrapper' ).each( function() {
				jQuery( this ).oxo_calc_flip_boxes_height();
			});

			// Make WooCommerce shortcodes work
			if ( clicked_tab.parents( '.oxo-tabs' ).find( tab_content_to_activate ).find( '.oxo-woo-slider' ).length ) {
				clicked_tab.parents( '.oxo-tabs' ).css( 'height', '' );
			}

			jQuery( '.crossfade-images' ).each(	function() {
				oxo_resize_crossfade_images_container( jQuery( this ) );
				oxoResizeCrossfadeImages( jQuery( this ) );
			});

			// Blog
			clicked_tab.parents( '.oxo-tabs' ).find( tab_content_to_activate ).find( '.oxo-blog-shortcode' ).each( function() {
				var columns = 2;
				for( i = 1; i < 7; i++ ) {
					if( jQuery( this ).find( '.oxo-blog-layout-grid' ).hasClass( 'oxo-blog-layout-grid-' + i ) ) {
						columns = i;
	...
	jQuery('.tooltip-shortcode, .oxo-secondary-header .oxo-social-networks li, .oxo-author-social .oxo-social-networks li, .oxo-footer-copyright-area .oxo-social-networks li, .oxo-footer-widget-area .oxo-social-networks li, .sidebar .oxo-social-networks li, .social_links_shortcode li, .share-box li, .social-icon, .social li').mouseenter(function(e){
	...
	jQuery('.oxo-portfolio .oxo-filters a').click( function( e ) {
		e.preventDefault();

		// Relayout isotope based on filter selection
		var $filter_active = jQuery( this ).data( 'filter' ),
			$lightbox_instances = [],
			$portfolio_id = jQuery( this ).parents( '.oxo-portfolio' ).data( 'id' );

		if ( ! $portfolio_id ) {
			$portfolio_id = '';
		}

		jQuery( this ).parents( '.oxo-portfolio' ).find( '.oxo-portfolio-wrapper' ).isotope( { filter: $filter_active } );

		// Remove active filter class from old filter item and add it to new
		jQuery( this ).parents( '.oxo-filters' ).find( '.oxo-filter' ).removeClass( 'oxo-active' );
		jQuery( this ).parent().addClass( 'oxo-active' );

		jQuery( this ).parents( '.oxo-portfolio' ).find( '.oxo-portfolio-wrapper' ).find( '.oxo-portfolio-post' ).each( function() {
			var $post_id = '';

			// For individual per post galleries set the post id
			if ( js_local_vars.lightbox_behavior == 'individual' && jQuery( this ).find( '.oxo-rollover-gallery' ).length ) {
				$post_id = jQuery( this ).find( '.oxo-rollover-gallery' ).data( 'id' );
			}
			...
			if ( $filter_active.length > 1 ) {
				...
				var $filter_selector = 'oxo-portfolio-post',
			...
			if ( jQuery( this ).hasClass( $filter_selector ) || $filter_active.length == 1 ) {
				...
				jQuery( this ).find( '.oxo-rollover-gallery' ).attr( 'data-rel', $lightbox_string );
				jQuery( this ).find( '.oxo-portfolio-gallery-hidden a' ).attr( 'data-rel', $lightbox_string );
			}
		});
		...
		if ( jQuery( this ).data( 'lightbox' ) != 'created' ) {
			...
			jQuery.each( $lightbox_instances, function( $key, $value ) {
				$il_instances.push( jQuery( '[data-rel="iLightbox[' + $value + ']"], [rel="iLightbox[' + $value + ']"]' ).iLightBox( $aione_lightbox.prepare_options( 'iLightbox[' + $value + ']' ) ) );
			});
	...
	jQuery( '.oxo-faqs' ).each( function() {
		// Initialize the filters and corresponding posts
		// Check if filters are displayed
		var $faqs_page = jQuery( this ),
			$filters_wrapper = $faqs_page.find( '.oxo-filters' );

			// Make the faq posts visible
			jQuery( '.oxo-faqs-wrapper' ).fadeIn();
		...
		if ( $filters_wrapper.length ) {
			...
			// Set needed variables
			var $filters = $filters_wrapper.find( '.oxo-filter' ),
				$filter_active_element = $filters_wrapper.find( '.oxo-active' ).children( 'a' ),
				$filter_active =  $filter_active_element.attr( 'data-filter' ).substr( 1 ),
				$posts = jQuery( this ).find( '.oxo-faqs-wrapper .oxo-faq-post' );
		...
		$faqs_page .find( '.oxo-filters a' ).click( function(e) {
			e.preventDefault();

			var selector = jQuery( this ).attr( 'data-filter' );

			// Fade out the faq posts and fade in the ones matching the selector
			$faqs_page .find( '.oxo-faqs-wrapper .oxo-faq-post' ).fadeOut();
			setTimeout( function() {
				$faqs_page .find( '.oxo-faqs-wrapper .oxo-faq-post' + selector ).fadeIn();
			}, 400 );
	...
	jQuery('.oxo-alert .close').click(function(e) {
		e.preventDefault();
	...
	if(Modernizr.mq('only screen and (max-width: 479px)')) {
		...
		jQuery('.oxo-portfolio .oxo-rollover-gallery').each(function () {
			var img = jQuery(this).attr('href');

			if(checkForImage(img) === true) {
				jQuery(this).parents('.oxo-image-wrapper').find('> img').attr('src', img).attr('width', '').attr('height', '');
			}
			jQuery(this).parents('.oxo-portfolio-post').css('width', 'auto');
			jQuery(this).parents('.oxo-portfolio-post').css('height', 'auto');
			jQuery(this).parents('.oxo-portfolio-one:not(.oxo-portfolio-one-text)').find('.oxo-portfolio-post').css('margin', '0');
		});

		if( jQuery('.oxo-portfolio').length ) {
			jQuery('.oxo-portfolio-wrapper').isotope();
		}
	}
	...
	jQuery( '.link-area-box' ).on('click', function() {
		if( jQuery( this ).attr( 'data-link' ) ) {
			...
			jQuery( this ).find( '.oxo-read-more' ).attr( 'target', '' );
		}
	});
	...
	jQuery( '.link-type-button' ).each(function() {
		if( jQuery( this ).parents( '.content-boxes-clean-vertical' ).length >= 1 ) {
			var $button_height = jQuery( '.oxo-read-more-button' ).outerHeight();
			jQuery( this ).find( '.oxo-read-more-button' ).css( 'top', $button_height / 2 );
		}
	});

	jQuery( '.link-area-link-icon .oxo-read-more-button, .link-area-link-icon .oxo-read-more, .link-area-link-icon .heading' ).mouseenter(function() {
		jQuery( this ).parents( '.link-area-link-icon' ).addClass( 'link-area-link-icon-hover' );
	});
	jQuery( '.link-area-link-icon .oxo-read-more-button, .link-area-link-icon .oxo-read-more, .link-area-link-icon .heading' ).mouseleave(function() {
    ...
    if(cssua.ua.mobile === undefined) {
	    ...
	    if(js_local_vars.page_title_fading == '1') {
		    if(js_local_vars.header_position == 'Left' || js_local_vars.header_position == 'Right') {
		    	jQuery('.oxo-page-title-wrapper').oxo_scroller({type: 'opacity', end_offset: '.oxo-page-title-captions > h1'});
		    } else {
			    jQuery('.oxo-page-title-wrapper').oxo_scroller({type: 'opacity', offset: 100});
			}
		}

		// Fading and blur effect for new fade="" param on full width boxes
	    jQuery('.fullwidth-faded').oxo_scroller({type: 'fading_blur'});
	}
});
...
jQuery(document).ready(function() {
	...
	jQuery('.oxo-fullwidth.video-background').each(function() {
...
jQuery(window).load(function() {
	jQuery('.oxo-youtube-flash-fix').remove();
});
...
var orig_menu_height = jQuery( '.header-wrapper .oxo-navbar-nav > li > a' ).outerHeight();
...
jQuery(window).load(function() {
	var headerHeight = jQuery( '.oxo-header-wrapper' ).height();

	if(jQuery('.sidebar').is(':visible')) {
		jQuery('.post-content .oxo-portfolio').each(function() {
			var columns = jQuery(this).data('columns');
			jQuery(this).addClass('oxo-portfolio-'+columns+'-sidebar');
		});
	}

	// Portfolio isotope loading
	if ( jQuery().isotope && jQuery( '.oxo-portfolio .oxo-portfolio-wrapper' ).length ) {
		jQuery( '.oxo-portfolio .oxo-portfolio-wrapper' ).each( function() {
			jQuery( this ).next( '.oxo-load-more-button' ).fadeIn();

			// Resize the placeholder images correctly in "fixed" picture size carousels
			if ( jQuery( this ).data( 'picturesize' ) == 'fixed' ) {
				jQuery( this ).find( '.oxo-placeholder-image' ).each( function() {
					jQuery( this ).css(	{
						'height': jQuery( this ).parents( '.oxo-portfolio-post' ).siblings().first().find( 'img' ).height(),
						'width': jQuery( this ).parents( '.oxo-portfolio-post' ).siblings().first().find( 'img' ).width()
					});
			...
			var $isotope_filter = '',
				$filters_container = jQuery( this ).parents( '.oxo-portfolio' ).find( '.oxo-filters' );

			// Check if filters are displayed
			if ( $filters_container.length ) {

				// Set needed variables
				var $filters = $filters_container.find( '.oxo-filter' ),
					$filter_active = $filters_container.find( '.oxo-active' ),
					$filter_active_link = $filter_active.children( 'a' ),
					$filter_active_data_slug = $filter_active_link.attr( 'data-filter' ).substr( 1 ),
					$posts = jQuery( this ).find( '.oxo-portfolio-post' ),
					$lightbox_instances = [];
							...
							$posts.each( function() {
								...
								if ( $filter_active_data_slug.length && $post.hasClass( $filter_active_data_slug ) ) {
									...
									if ( $post_gallery_name ) {
										...
										$post.find( '.oxo-rollover-gallery' ).attr( 'data-rel', $lightbox_filter );
				...
				if ( $filter_active_data_slug.length ) {
					...
					// Create new lightbox instance for the new galleries
					jQuery.each( $lightbox_instances, function( $key, $value ) {
						$il_instances.push( jQuery( '[data-rel="' + $value + '"], [rel="' + $value + '"]' ).iLightBox( $aione_lightbox.prepare_options( $value ) ) );
					});
			...
			setTimeout( function() {
				...
				if ( $portfolio_wrapper.parent().hasClass( 'oxo-portfolio-one' ) ) {
					window.$portfolio_isotope = $portfolio_wrapper;
					window.$portfolio_isotope.isotope({
						// Isotope options
						itemSelector: '.oxo-portfolio-post',
						layoutMode: 'vertical',
			...
			var $placeholder_images = jQuery( this ).find( '.oxo-portfolio-post .oxo-placeholder-image' );
			$placeholder_images.each( function() {
				jQuery( this ).parents( '.oxo-portfolio-content-wrapper, .oxo-image-wrapper' ).animate({ opacity: 1 });
			});

			// Fade in videos
			var $videos = jQuery( this ).find( '.oxo-portfolio-post .oxo-video' );
			$videos.each( function() {
				jQuery( this ).animate({ opacity: 1 });
				jQuery( this ).parents( '.oxo-portfolio-content-wrapper' ).animate({opacity: 1});
			});
			...
			jQuery( this ).imagesLoaded().progress( function( $instance, $image ) {
				if( jQuery( $image.img ).parents( '.oxo-portfolio-content-wrapper' ).length >= 1 ) {
					jQuery( $image.img, $placeholder_images ).parents( '.oxo-portfolio-content-wrapper' ).delay( 100 * window.$portfolio_images_index ).animate({
						opacity: 1
					});
				} else {
					jQuery( $image.img, $placeholder_images ).parents( '.oxo-image-wrapper' ).delay( 100 * window.$portfolio_images_index ).animate({
						opacity: 1
					});
				}
	...
	if(jQuery().flexslider) {
		...
		jQuery('.tfs-slider').each(function() {
			...
			if(jQuery(this_tfslider).parents('#sliders-container').length >= 1 && jQuery(this_tfslider).data('parallax') === 1) {
				jQuery('.oxo-header').addClass('oxo-header-backface');
			}

			if(jQuery(this_tfslider).data('full_screen') == 1) {
				var sliderHeight = jQuery(window).height();

				if(jQuery(this_tfslider).parents('#sliders-container').next().hasClass('oxo-header-wrapper')) {
			...
			jQuery(this_tfslider).parents('.oxo-slider-container').css('height', sliderHeight);
			...
			if(cssua.ua.mobile) {
				jQuery(this_tfslider).find('.oxo-button').each(function() {
			...
			jQuery(window).on('resize', function() { // start_tfslider_resize
				if( jQuery(window).width() != resize_width || jQuery(window).height() != resize_height ) {
					...
					jQuery(this_tfslider).parents('.oxo-slider-container').css('height', sliderHeight);
					jQuery(this_tfslider).parents('.oxo-slider-container').css('max-height', sliderHeight );
					jQuery(this_tfslider).css('height', sliderHeight);
					jQuery(this_tfslider).find('.background, .mobile_video_image').css('height', sliderHeight);
					...
					if(jQuery(this_tfslider).data('parallax') == 1 && ! Modernizr.mq('only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)')) {
						...
						if( jQuery( '.oxo-header-wrapper' ).css( 'position' ) != 'absolute' ) {
							jQuery('.oxo-header-wrapper').css('position', 'relative');
							jQuery(this_tfslider).parents('.oxo-slider-container').css('margin-top', '-' + headerHeight + 'px');
						}
						jQuery('#main, .oxo-footer-widget-area, .oxo-footer-copyright-area, .oxo-page-title-bar').css('position', 'relative');
						jQuery('#main, .oxo-footer-widget-area, .oxo-footer-copyright-area, .oxo-page-title-bar').css('z-index', '3');
						jQuery('.oxo-header-wrapper').css('z-index', '5');
						jQuery('.oxo-header-wrapper').css('height', headerHeight);
						...
						jQuery('#main, .oxo-footer-widget-area, .oxo-footer-copyright-area, .oxo-page-title-bar').css('z-index', '3');
						jQuery('.oxo-header-wrapper').css('z-index', '5');
						jQuery('.oxo-header-wrapper').css('height', 'auto');
						jQuery(this_tfslider).parents('.oxo-slider-container').css('margin-top', '');
					...
					if(jQuery(this_tfslider).parents('.post-content').length >= 1) {
						jQuery(this_tfslider).parents('.oxo-slider-container').css('height', 'auto');
						jQuery(this_tfslider).css('height', 'auto');
						jQuery(this_tfslider).parents('.oxo-slider-container').css('max-height', 'none');
					...
					if(Modernizr.mq('only screen and (max-width: ' + js_local_vars.side_header_break_point + 'px)')) {
							jQuery('.oxo-header-wrapper').css('height', '' );
					}
			...
			jQuery(this_tfslider).flexslider({
				...
				start: function(slider) {
					jQuery( this_tfslider ).parent().find( '.oxo-slider-loading' ).remove();
					...
					// Remove title separators and padding, when there is not enough space
					jQuery( slider.slides.eq( slider.currentSlide ) ).find( '.oxo-title' ).oxo_responsive_title_shortcode();
...
*/
					...
					oxo_reanimate_slider( slideContent );
		...
		jQuery('.oxo-blog-layout-grid .flexslider').flexslider({
			...
			nextText: '&#xf105;',
			start: function(slider) {
				jQuery( slider ).removeClass( 'oxo-flexslider-loading' );
		...
		jQuery('.oxo-flexslider').flexslider({
			slideshow: Boolean(Number(js_local_vars.slideshow_autoplay)),
			...
			start: function(slider) {
				// Remove Loading
				slider.removeClass('oxo-flexslider-loading');
	...
	if(jQuery().isotope) {
		...
		jQuery( '.oxo-blog-layout-grid' ).each(function() {
			$grid_container = jQuery( this );

			var columns = 2;
			for( i = 1; i < 7; i++ ) {
				if( jQuery ( this ).hasClass( 'oxo-blog-layout-grid-' + i ) ) {
					columns = i;
				}
			}

			var grid_width = Math.floor( 100 / columns * 100 ) / 100  + '%';
			$grid_container.find( '.oxo-post-grid' ).css( 'width', grid_width );
			$grid_container.isotope({
				layoutMode: 'masonry',
				itemSelector: '.oxo-post-grid',
				transformsEnabled: false,
			...
			if( ( $grid_container.hasClass( 'oxo-blog-layout-grid-4') || $grid_container.hasClass( 'oxo-blog-layout-grid-5') || $grid_container.hasClass( 'oxo-blog-layout-grid-6') ) && Modernizr.mq('only screen and (min-device-width: 768px) and (max-device-width: 1024px) and (orientation: portrait)')) {
	...
	if ( Boolean( Number( js_local_vars.aione_rev_styles ) ) ) {
		jQuery( '.rev_slider_wrapper' ).each(function() {
			...
			if ( rev_slider_wrapper.length >=1 && rev_slider_wrapper.attr( 'class' ).indexOf( 'tp-shadow' ) == -1 ) {
				...
				rev_slider_wrapper.addClass( 'aione-skin-rev' );
			}
			...
			if( ! jQuery(this).find( '.tp-leftarrow' ).hasClass( 'preview1' ) && ! jQuery(this).find( '.tp-leftarrow' ).hasClass( 'preview2' ) && ! jQuery(this).find( '.tp-leftarrow' ).hasClass( 'preview3' ) && ! jQuery(this).find( '.tp-leftarrow' ).hasClass( 'preview4' ) ) {
				...
				if( rev_slider_wrapper.find( '.tp-leftarrow' ).height() > rev_slider_wrapper.height() / 4 && rev_slider_wrapper.find( '.tp-leftarrow' ).height() < rev_slider_wrapper.height() ) {
					...
					if( rev_slider_wrapper.children( '.aione-rev-arrows' ).length ) {
						rev_slider_wrapper.children( '.aione-rev-arrows' ).empty();
						rev_slider_wrapper.children( '.aione-rev-arrows' ).append( '<style type="text/css">#' + rev_slider_id + ' .tp-leftarrow, #' + rev_slider_id + ' .tp-rightarrow{margin-top:-' + new_dimension / 2 + 'px !important;width:' + new_dimension + 'px !important;height:' + new_dimension + 'px !important;}#' + rev_slider_id + ' .tp-leftarrow:before, #' + rev_slider_id + ' .tp-rightarrow:before{line-height:' + new_dimension  + 'px;font-size:' + new_dimension / 2 + 'px;}</style>' );
					} else {
						rev_slider_wrapper.prepend( '<div class="aione-rev-arrows"><style type="text/css">#' + rev_slider_id + ' .tp-leftarrow, #' + rev_slider_id + ' .tp-rightarrow{margin-top:-' + new_dimension / 2 + 'px !important;width:' + new_dimension + 'px !important;height:' + new_dimension + 'px !important;}#' + rev_slider_id + ' .tp-leftarrow:before, #' + rev_slider_id + ' .tp-rightarrow:before{line-height:' + new_dimension  + 'px;font-size:' + new_dimension / 2 + 'px;}</style></div>' );
					}
				}
				...
				jQuery(window).on('resize', function() {
					if( rev_slider_wrapper.find( '.tp-leftarrow' ).height() > rev_slider_wrapper.height() / 4 && rev_slider_wrapper.find( '.tp-leftarrow' ).height() < rev_slider_wrapper.height() ) {
						...
						rev_slider_wrapper.children( '.aione-rev-arrows' ).remove();
					}
				});
			}
...
jQuery( document ).ready( function() {
	...
	if(jQuery().flexslider && jQuery('.flexslider-attachments').length >= 1) {
		...
		jQuery('.flexslider-attachments').flexslider({
			...
			start: function(slider) {
				jQuery( slider ).find( '.oxo-slider-loading' ).remove();
...
jQuery( window ).load( function() {
	setTimeout( function() {
		var cart_width = jQuery( '.oxo-secondary-menu .oxo-secondary-menu-cart' ).outerWidth(),
			cart_content = jQuery('.oxo-secondary-menu-cart .oxo-menu-cart-items a'),
			...
			img = jQuery( '.oxo-secondary-menu-cart .oxo-menu-cart-items a > img' ),
			img_width = img.width();
			img_margin = img.outerWidth( true ) - img_width;

		jQuery('.oxo-secondary-menu-cart .oxo-secondary-menu-icon').css( 'min-width', cart_width );
		jQuery('.oxo-secondary-menu-cart .oxo-menu-cart-items').width( cart_width );
		jQuery('.oxo-secondary-menu-cart .oxo-menu-cart-items a').width( new_cart_content_width );

		jQuery('.oxo-secondary-menu-cart .oxo-menu-cart-items a').find( '.oxo-menu-cart-item-details' ).each( function() {
	...
	if(js_local_vars.custom_icon_image_retina && retina) {
		jQuery('.oxo-social-networks a.custom').each(function() {
	...
	var last_timeline_date = jQuery( '.oxo-blog-layout-timeline' ).find( '.oxo-timeline-date' ).last().text();
	var collapse_month_visible = true;

	jQuery( '.oxo-blog-layout-timeline' ).find( '.oxo-timeline-date' ).click( function() {
		jQuery( this ).next( '.oxo-collapse-month' ).slideToggle();
	});

	jQuery( '.oxo-timeline-icon' ).find( '.oxo-icon-bubbles' ).click( function() {
		if ( collapse_month_visible ) {
			jQuery( this ).parent().next( '.oxo-blog-layout-timeline' ).find( '.oxo-collapse-month' ).slideUp();
			collapse_month_visible = false;
		} else {
			jQuery( this ).parent().next( '.oxo-blog-layout-timeline' ).find( '.oxo-collapse-month' ).slideDown();
			collapse_month_visible = true;
		}
	});

	// Setup infinite scroll for each blog instance; main blog page and blog shortcodes
	jQuery( '.oxo-posts-container-infinite' ).each( function() {
		...
		if ( jQuery( this ).find( '.oxo-blog-layout-timeline' ).length ) {
			$blog_infinite_container = jQuery( this ).find( '.oxo-blog-layout-timeline' );
		}

		// If more than one blog shortcode is on the page, make sure the infinite scroll selectors are correct
		$parent_wrapper_classes = '';
		if ( $blog_infinite_container.parents( '.oxo-blog-shortcode' ).length ) {
			$parent_wrapper_classes = '.' + $blog_infinite_container.parents( '.oxo-blog-shortcode' ).attr( 'class' ).replace( /\ /g, '.' ) + ' ';
		}
		...
		jQuery( $blog_infinite_container ).infinitescroll({
			...
			itemSelector : $parent_wrapper_classes + 'div.pagination .current, ' + $parent_wrapper_classes + 'div.post:not( .oxo-archive-description ), ' + $parent_wrapper_classes + '.oxo-collapse-month, ' + $parent_wrapper_classes + '.oxo-timeline-date',
						   // selector for all items you'll retrieve
			loading	  : {
							finishedMsg: js_local_vars.infinite_finished_msg,
							msg: jQuery( '<div class="oxo-loading-container oxo-clearfix"><div class="oxo-loading-spinner"><div class="oxo-spinner-1"></div><div class="oxo-spinner-2"></div><div class="oxo-spinner-3"></div></div><div class="oxo-loading-msg">'+js_local_vars.infinite_blog_text+'</div>' )
			},
			...
			if ( jQuery( $blog_infinite_container ).hasClass( 'oxo-blog-layout-timeline' ) ) {
				// Check if the last already displayed moth is the same as the first newly loaded; if so, delete one label
				if ( jQuery( posts ).first( '.oxo-timeline-date' ).text() == last_timeline_date ) {
					jQuery( posts ).first( '.oxo-timeline-date' ).remove();
				}
				// Set the last timeline date to lat of the currently loaded
				last_timeline_date = jQuery( $blog_infinite_container ).find( '.oxo-timeline-date' ).last().text();

				// Append newly loaded items of the same month to the container that is already there
				jQuery( $blog_infinite_container ).find( '.oxo-timeline-date' ).each( function() {
					jQuery( this ).next( '.oxo-collapse-month' ).append( jQuery( this ).nextUntil( '.oxo-timeline-date', '.oxo-post-timeline' ) );
				});

				// If all month containers are collapsed, also collapse the new ones
				if ( ! collapse_month_visible ) {
					setTimeout( function() {
						jQuery( $blog_infinite_container ).find ( '.oxo-collapse-month' ).hide();
					}, 200 );
				}

				// Delete empty collapse-month containers
				setTimeout( function() {
					jQuery( $blog_infinite_container ).find ( '.oxo-collapse-month' ).each( function() {
				...
				jQuery( $blog_infinite_container ).find( '.oxo-timeline-date' ).unbind( 'click' );
				jQuery( $blog_infinite_container ).find( '.oxo-timeline-date' ).click( function() {
					jQuery( this ).next( '.oxo-collapse-month' ).slideToggle();
				});
			}

			// Grid layout specific actions
			if ( jQuery( $blog_infinite_container ).hasClass( 'oxo-blog-layout-grid' ) &&
				 jQuery().isotope
			) {
				jQuery( posts ).hide();
				...
				var columns = 2;
				for ( i = 1; i < 7; i++ ) {
					if ( jQuery( $blog_infinite_container ).hasClass( 'oxo-blog-layout-grid-' + i ) ) {
						columns = i;
					}
				}
			...
			$oxo_posts_container = $blog_infinite_container;
			if ( jQuery( $blog_infinite_container ).hasClass( 'oxo-blog-layout-timeline' ) ) {
				$oxo_posts_container = jQuery( $blog_infinite_container ).parents( '.oxo-posts-container-infinite' );
			}

			$current_page = $oxo_posts_container.find( '.current' ).html();
			$oxo_posts_container.find( '.current' ).remove();

			if ( $oxo_posts_container.data( 'pages' ) == $current_page ) {
				$oxo_posts_container.parent().find( '.oxo-loading-container' ).hide();
           		$oxo_posts_container.parent().find( '.oxo-load-more-button' ).hide();
			}

			// Activate lightbox for the newly added posts
			if( js_local_vars.lightbox_behavior == 'individual' || ! $original_posts.find( '.oxo-post-slideshow' ).length ) {
				$aione_lightbox.activate_lightbox( jQuery( posts ) );
		...
		if ( ( jQuery( $blog_infinite_container ).hasClass( 'oxo-blog-archive' ) && js_local_vars.blog_pagination_type == 'load_more_button' ) ||
			 jQuery( $blog_infinite_container ).hasClass( 'oxo-posts-container-load-more' ) ||
			 ( jQuery( $blog_infinite_container ).hasClass( 'oxo-blog-layout-timeline' ) && jQuery( $blog_infinite_container ).parent().hasClass( 'oxo-posts-container-load-more' ) )
		) {
			jQuery( $blog_infinite_container ).infinitescroll( 'unbind' );

			// Load more posts button click
			if ( jQuery( $blog_infinite_container ).hasClass( 'oxo-blog-archive' ) ) {
				$load_more_button = jQuery( $blog_infinite_container ).parent().find( '.oxo-load-more-button' );
			} else {
				$load_more_button = jQuery( $blog_infinite_container ).parents( '.oxo-blog-archive' ).find( '.oxo-load-more-button' );
			}
		...
		if ( $oxo_posts_container.data( 'pages' ) == '1' ) {
			$oxo_posts_container.parent().find( '.oxo-loading-container' ).hide();
			$oxo_posts_container.parent().find( '.oxo-load-more-button' ).hide();
		}
	});
	...
	if ( js_local_vars.portfolio_pagination_type == 'Infinite Scroll' || js_local_vars.portfolio_pagination_type == 'load_more_button' ) {
		$infinte_scroll_container = jQuery( '.oxo-portfolio:not(.oxo-recent-works) .oxo-portfolio-wrapper' );
		...
		$infinte_scroll_container.infinitescroll({
			...
			nextSelector : ".pagination-next",
						   // selector for the NEXT link (to page 2)
			itemSelector : 'div.pagination .current, .oxo-portfolio-post',
						   // selector for all items you'll retrieve
			loading	  : {
							finishedMsg: js_local_vars.infinite_finished_msg,
							msg: jQuery('<div class="oxo-loading-container oxo-clearfix"><div class="oxo-loading-spinner"><div class="oxo-spinner-1"></div><div class="oxo-spinner-2"></div><div class="oxo-spinner-3"></div></div><div class="oxo-loading-msg">'+js_local_vars.infinite_blog_text+'</div>')
			...
			errorCallback: function() {
				jQuery( '.oxo-portfolio .oxo-portfolio-wrapper' ).isotope();
			},
			...
			if ( jQuery().isotope ) {
				...
				var $filters = jQuery( '.oxo-filters' ).find( '.oxo-filter' ),
					$posts = jQuery( $posts );
				...
				imagesLoaded( $posts, function() {
					// Fade in placeholder images
					var $placeholder_images = jQuery( $posts ).find( '.oxo-placeholder-image' );
					$placeholder_images.parents( '.oxo-portfolio-content-wrapper, .oxo-image-wrapper' ).animate({ opacity: 1 });

					// Fade in videos
					var $videos = jQuery( $posts ).find( '.oxo-video' );
					$videos.each( function() {
						jQuery( this ).animate({ opacity: 1 });
						jQuery( this ).parents( '.oxo-portfolio-content-wrapper' ).animate({opacity: 1});
					});
					...
					// Check if filters are displayed
				 	if ( jQuery( '.oxo-filters' ).length ) {
						// Display new posts based on filter selection
						var $filter_active_element = jQuery( '.oxo-portfolio .oxo-filters' ).find( '.oxo-filter.oxo-active a' ),
							$filter_active = $filter_active_element.attr( 'data-filter' ).substr( 1 );
						...
						if ( $filter_active.length ) {
							...
							if ( $filter_active_element.data( 'lightbox' ) != 'created' ) {
								...
								// Create new lightbox instance for the new gallery
								$il_instances.push( jQuery( '[data-rel^="iLightbox[' + $filter_active + ']"]' ).iLightBox( $aione_lightbox.prepare_options( 'iLightbox[' + $filter_active + ']' ) ) );
					...
					if ( $infinte_scroll_container.data( 'pages' ) == $current_page ) {
						$infinte_scroll_container.parent().find( '.oxo-loading-container' ).hide();
						$infinte_scroll_container.parent().find( '.oxo-load-more-button' ).hide();
					}
				});
			}
		});
		...
		if ( js_local_vars.portfolio_pagination_type == 'load_more_button' ) {
			...
			jQuery( '.oxo-portfolio' ).find( '.oxo-load-more-button' ).on( 'click', function(e) {
				e.preventDefault();
...
if ( location.hash && location.hash.substring( 0, 2 ) === '#_' ) {
	...
	jQuery( '.oxo-page-load-link' ).attr( 'href', '#' + $hash );

	// Scroll the page
	jQuery( document ).ready( function() {
		jQuery( '.oxo-page-load-link' ).trigger( 'click' );
	});
}
