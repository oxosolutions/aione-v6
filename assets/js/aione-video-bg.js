...
function resizeVideo( $wrapper ) {
	...
	if ( $videoContainer.find( '.oxo-video-cover' ).length < 1 ) {
		var $parent = $videoContainer.find( 'iframe' ).parent();
		$parent.prepend( '<div class="oxo-video-cover">&nbsp;</div>' );
	}
	// No YouTube right click stuff!
	$videoContainer.find( '.oxo-video-cover' ).css(
		{
			'z-index': 0,
			'width': width,
			'height': height,
...
jQuery( document ).ready(
	function( $ ) {
		...
		$( '.bg-parallax.video, .oxo-bg-parallax.video' ).each(
			function() {
