...
if ( typeof _oxoImageParallaxImages === 'undefined' ) {
    var _oxoImageParallaxImages = [];
    var _oxoScrollTop;
    var _oxoWindowHeight;
    var _oxoScrollLeft;
    var _oxoWindowWidth;
}
...
(function( $, window, document, undefined ) {
    ...
    var pluginName = "oxoImageParallax",
        defaults = {
            direction: 'up', // fixed
            mobileenabled: false,
            mobiledevice: false,
    ...
    function Plugin( element, options ) {
		...
		// OXOSolutions edit for Aione theme: making background position work
    ...
    $.extend(
        Plugin.prototype, {
            init: function() {
                ...
                _oxoImageParallaxImages.push( this );
...
*/
                ...
                var percentageScroll = (_oxoScrollTop - this.scrollTopMin) / (this.scrollTopMax - this.scrollTopMin);
                ...
                if ( typeof _oxoParallaxIE9 !== 'undefined' ) {
...
/*
                 	$target.css({
                 		...
                 		webkitTransform: translateVert + _oxoScrollTop + translateVertSuffix,
                 		mozTransform: translateVert + _oxoScrollTop + translateVertSuffix,
                 		msTransform: translateVert + _oxoScrollTop + translateVertSuffix,
                 		oTransform: translateVert + _oxoScrollTop + translateVertSuffix,
                 		transform: translateVert + _oxoScrollTop + translateVertSuffix
                	});
*/
                }
            ...
            isInView: function() {
                ...
                if ( elemTop + elemHeight < _oxoScrollTop || _oxoScrollTop + _oxoWindowHeight < elemTop ) {
                    return false;
                }
...
/*
                     // Stretch the image to fit the entire window
					var w = $target.width() + parseInt( $target.css( 'paddingRight' ) ) + parseInt( $target.css( 'paddingLeft' ) );
					var h = _oxoWindowHeight;

                 	if ( this.isMobile ) {
						h += 70;
						w = h / _oxoWindowHeight * w;
					}
					...
					$target.css({
						...
						overflow: 'hidden',
						zIndex: 1,
						'background-image': 'none' // OXOSolutions edit for Aione theme: fxing background-image duplication
					})
					...
					.find('.parallax-inner').css({
						...
						top: - this.settings.target.offset().top,
						left: 0,
						backgroundPosition: this.settings.xpos + ' ' + this.settings.ypos, // OXOSolutions edit for Aione theme: fxing bg position
						backgroundAttachment: 'scroll',
						backgroundRepeat: isRepeat ? 'repeat' : 'no-repeat',
						backgroundSize: 'cover' // OXOSolutions edit for Aione theme: make the bg image stretch over all container width
					});
...
*/
                    ...
                    $target.css(
                        ...
                        .find( '.parallax-inner' ).css(
                        {
                            ...
                            left: left,
                            opacity: Math.abs( parseFloat( this.settings.opacity ) / 100 ),
                            backgroundPosition: isRepeat ? '0 0 ' : this.settings.xpos + ' ' + this.settings.ypos, // OXOSolutions edit for Aione theme: fxing bg position
                            backgroundRepeat: isRepeat ? 'repeat' : 'no-repeat',
                            backgroundSize: isRepeat ? 'auto' : 'cover' // OXOSolutions edit for Aione theme: make the bg image stretch over all container width
                        }
                    );
                        ...
                        $target.find( '.parallax-inner' ).css(
                            {
                                opacity: Math.abs( parseFloat( this.settings.opacity ) / 100 ),
                                backgroundImage: 'url(' + this.settings.image + ')',
                            	// OXOSolutions edit for Aione theme: IE 8 background-size: cover filter
                    ...
                    var scrollTopMin = 0;
                    if ( $target.offset().top > _oxoWindowHeight ) {
                        scrollTopMin = $target.offset().top - _oxoWindowHeight;
                    }
			...
			// OXOSolutions edit for Aione theme: completely new mobile check
			isMobile: function() {
...
function _oxoRefreshScroll() {
    var $ = jQuery;
    _oxoScrollTop = window.pageYOffset;//$(window).scrollTop();
    _oxoScrollLeft = window.pageXOffset;//$(window).scrollLeft();
}

function _oxoParallaxAll() {
    _oxoRefreshScroll();
    for ( var i = 0; i < _oxoImageParallaxImages.length; i++ ) {
        _oxoImageParallaxImages[i].doParallax();
    }
}

// OXOSolutions edit for Aione theme: moved function out of ready event
function _oxoRefreshWindow() {
	_oxoScrollTop = window.pageYOffset;//$(window).scrollTop();
	_oxoWindowHeight = jQuery( window ).height();
	_oxoScrollLeft = window.pageXOffset;//$(window).scrollLeft();
	_oxoWindowWidth = jQuery( window ).width();
}
    ...
    function( $ ) {
        ...
        $( window ).on(
            'scroll touchmove touchstart touchend gesturechange', function( e ) {
                requestAnimationFrame( _oxoParallaxAll );
            }
        );
            ...
            'resize', function() {
                setTimeout(
                    function() {
                        ...
                        _oxoRefreshWindow();
                        $.each(
                            _oxoImageParallaxImages, function( i, parallax ) {
...
// OXOSolutions edit for Aione theme: needed if OxoSlider is present to recalc the dimensions
...
// @codekit-prepend "aione-parallax.js"
// @codekit-append "aione-video-bg.js"
...
jQuery( document ).ready( function( $ ) {
    ...
    if ( _isMobile() ) {
        $( '.oxo-bg-parallax.video > div' ).remove();
    }

    // Hide the placeholder
    $('.oxo-bg-parallax').next().addClass('bg-parallax-parent');
    $('.oxo-bg-parallax').attr('style', '').css('display', 'none');
    ...
    $( '.oxo-bg-parallax' ).each(function() {
        $(this).oxoImageParallax({
	...
	// OXOSolutions edit for Aione theme: remove IE 8 background-size: cover filter for all others
	if ( ! jQuery( '.ua-ie-8' ).length ) {
		$( '.oxo-parallax-fixed' ).each( function() {
			$( this ).css({
				'filter': '',
				'-ms-filter': '',
			});
